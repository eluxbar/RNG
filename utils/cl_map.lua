globalBlips = {}
function tRNG.addBlip(a, b, c, d, e, f, g, h)
...
function tRNG.removeBlip(j)
    RemoveBlip(j)
end
function tRNG.removeBlipAtCoords(x,y,z) -- used to remove green house blips for everyone when purchased
...
local k = {}
function tRNG.setNamedBlip(l, a, b, c, d, e, f, g)
    tRNG.removeNamedBlip(l)
    k[l] = tRNG.addBlip(a, b, c, d, e, f, g)
    return k[l]
end
function tRNG.removeNamedBlip(l)
    if k[l] ~= nil then
        tRNG.removeBlip(k[l])
        k[l] = nil
    end
end
function tRNG.setGPS(a, b)
    SetNewWaypoint(a + 0.0001, b + 0.0001)
end
function tRNG.setBlipRoute(j)
    SetBlipRoute(j, true)
end
local m = {}
...
local p = {}
local q = {}
function tRNG.addMarker(a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    local I = {
...
function tRNG.setNamedMarker(l, a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    tRNG.removeNamedMarker(l)
    o[l] = tRNG.addMarker(a, b, c, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, G, H)
    return o[l]
end
function tRNG.removeNamedMarker(l)
    if o[l] ~= nil then
        tRNG.removeMarker(o[l])
        o[l] = nil
    end
end
local J = {}
...
Citizen.CreateThread(
    function()
        while true do
            q = tRNG.getNearbyMarkers()
            Citizen.Wait(10000)
        end
    end
)
function tRNG.getNearbyMarkers()
    local N = {}
    local O = tRNG.getPlayerCoords()
    local P = 0
...
function tRNG.setArea(l, a, b, c, T, U)
...
function tRNG.createArea(l, W, T, U, X, Y, Z, _)
...
function tRNG.removeArea(l)
    if Q[l] then
        Q[l] = nil
    end
end
function tRNG.doesAreaExist(l)
    if Q[l] then
        return true
    end
    return false
end
function tRNG.setAreaMetaData(l, _)
    if Q[l] then
        Q[l].metaData = _
    end
end
function tRNG.getAreaMetaData(l)
    if Q[l] then
        return Q[l].metaData
    else
        return {}
        ...
        for a0, a1 in pairs(R) do
            ...
            if a1.player_in and not a3 then
                if a1.leaveArea then
                    ...
                    TriggerServerEvent("RNG:leaveArea", a0)
                end
            elseif not a1.player_in and a3 then
                if a1.enterArea then
                    ...
                    TriggerServerEvent("RNG:enterArea", a0)
                end
            end
            a1.player_in = a3
        end
...
RegisterCommand("nextblip",function()
    ...
    a4 = tRNG.addBlip(1103.9739990234, 211.95138549805, -49.440101623535, a5, 0, "Chips Cashier", 0.8, true)
end)
