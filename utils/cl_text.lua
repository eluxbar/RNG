function tRNG.add3DTextForCoord(a, b, c, d, e)
    local function f(g)
        tRNG.DrawText3D(g.coords, g.text, 0.2)
    end
    local h = tRNG.generateUUID("3dtext", 8, "alphanumeric")
    tRNG.createArea("3dtext_" .. h,vector3(b, c, d),e,6.0,function()end,function()end,f,{coords = vector3(b, c, d), text = a})
end
function tRNG.drawFloatingHelpText(a, i)
...
function tRNG.DrawText3D(i, a, j, k, l, m)
...
function tRNG.DrawSprite3d(data)
...
function tRNG.DrawText(b, c, a, j, k, p, m, q)
    SetTextScale(j, j)
    SetTextFont(k or 0)
    if p then
