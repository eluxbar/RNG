...
function RNG.getUserGroups(user_id)
...
function RNG.addUserGroup(user_id, group)
    if not RNG.hasGroup(user_id, group) then
        local user_groups = RNG.getUserGroups(user_id)
        ...
        if ngroup then
            ...
            TriggerEvent("RNG:playerJoinGroup", user_id, group, gtype)
        end
    end
end
...
function RNG.getUserGroupByType(user_id, gtype)
    local user_groups = RNG.getUserGroups(user_id)
    for k, v in pairs(user_groups) do
        local kgroup = groups[k]
        if kgroup then
...
function RNG.getUsersByGroup(group)
    local users = {}

    for k, v in pairs(RNG.rusers) do
        if RNG.hasGroup(tonumber(k), group) then
...
function RNG.getUsersByPermission(perm)
    local users = {}

    for k, v in pairs(RNG.rusers) do
        if RNG.hasPermission(tonumber(k), perm) then
            table.insert(users, tonumber(k))
        end
    end
...
function RNG.removeUserGroup(user_id, group)
    local user_groups = RNG.getUserGroups(user_id)
    ...
    TriggerEvent("RNG:playerLeaveGroup", user_id, group, gtype)

    user_groups[group] = nil -- remove reference
end

-- check if the user has a specific group
function RNG.hasGroup(user_id, group)
    local user_groups = RNG.getUserGroups(user_id)
...
-- check if the user has a specific permission
function RNG.hasPermission(user_id, perm)
    local user_groups = RNG.getUserGroups(user_id)
    ...
    if fchar == "@" then -- special aptitude permission
        ...
        if #parts == 3 then -- decompose group.aptitude.operator
            ...
            local afr = math.floor(RNG.expToLevel(RNG.getExp(user_id, group, aptitude)))

            local fop = string.sub(op, 1, 1)
            if fop == "<" then -- less (group.aptitude.<x)
                local rng = parseInt(string.sub(op, 2, string.len(op)))
                if afr < rng then
                    return true
                end
            elseif fop == ">" then -- greater (group.aptitude.>x)
                ...
                local rng = parseInt(string.sub(op, 1, string.len(op)))
                if afr == rng then
                    return true
                end
            end
    ...
    elseif fchar == "#" then -- special item permission
        ...
        if #parts == 2 then -- decompose item.operator
            ...
            local amount = RNG.getInventoryItemAmount(user_id, item)
...
function RNG.hasPermissions(user_id, perms)
...
exports('hasPermission', function(source, perm)
    return RNG.hasPermission(RNG.getUserId(source), perm)
end)

exports('hasGroup', function(source, group)
    return RNG.hasGroup(RNG.getUserId(source), group)
end)