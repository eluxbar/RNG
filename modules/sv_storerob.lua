...
RegisterNetEvent('RNG:getStoreRobBlips')
AddEventHandler('RNG:getStoreRobBlips', function()
    TriggerClientEvent('RNG:updateStoreRobBlips', source, stores)
end)


RegisterNetEvent('RNG:initiateStoreRobbery')
AddEventHandler('RNG:initiateStoreRobbery', function(store)
    local source = source
    local user_id = RNG.getUserId(source)
    if RNG.hasPermission(user_id, "police.armoury") then
        TriggerClientEvent('RNG:resetStorePed', source, store)
    else
        if #RNG.getUsersByPermission('police.armoury') >= 0 then
            for k,v in pairs(stores) do
                if k == store then
                    if v.cooldown == 0 then
                        v.beingrobbed = true
                        v.cooldown = 300
                        TriggerClientEvent('RNG:updateStoreRobBlips', -1, stores)
                        TriggerClientEvent('RNG:beginStoreRobbingAnimations', -1, store)
                        TriggerClientEvent('RNG:storeRobberyInProgress', source, true, store)
                        ...
                        TriggerEvent('RNG:PDRobberyCall', source, v.storename, v.position)
                    else
            ...
            RNGclient.notify(source, {'~r~There are not enough police on duty to rob a store.'})
            TriggerClientEvent('RNG:resetStorePed', source, store)
        end
    end
end)

RegisterNetEvent('RNG:completeSafeCracking')
AddEventHandler('RNG:completeSafeCracking', function(store)
    local source = source
    local user_id = RNG.getUserId(source)
    RNG.giveDirtyCash(user_id, math.random(85000, 150000))
    TriggerClientEvent('RNG:syncCloseSafeDoor', -1, store)
    TriggerClientEvent('RNG:resetStorePed', -1, store)
    for k,v in pairs(stores) do
        if k == store then
            v.beingrobbed = false
        end
    ...
    TriggerClientEvent('RNG:updateStoreRobBlips', -1, stores)
    TriggerClientEvent('RNG:storeRobberyInProgress', source, false, store)
end)

RegisterNetEvent('RNG:forceEndRobbery')
AddEventHandler('RNG:forceEndRobbery', function(store)
    local source = source
    local user_id = RNG.getUserId(source)
    TriggerClientEvent('RNG:resetStorePed', -1, store)
    for k,v in pairs(stores) do
...
RegisterNetEvent('RNG:syncOpenSafeDoor')
AddEventHandler('RNG:syncOpenSafeDoor', function(store)
    local source = source
    TriggerClientEvent('RNG:syncOpenSafeDoor', -1, store)
end)
