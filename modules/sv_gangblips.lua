...
RegisterServerEvent("RNG:EnableGangBlips")
AddEventHandler("RNG:EnableGangBlips", function()
    local source = source
    ...
    if playerGang then
        ...
        for _, memberID in ipairs(gangMembers) do
            if memberID ~= user_id and RNG.isPlayerOnline(memberID) then
                table.insert(filteredMembers, memberID)
            end
        end

        local blipsData = GetBlipsData(filteredMembers)
        TriggerClientEvent("RNG:ReceiveGangBlipsData", source, blipsData)
    end
end)
