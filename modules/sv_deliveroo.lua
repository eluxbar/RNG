...
RegisterNetEvent("RNG:attemptBeginDeliverooJob")
AddEventHandler("RNG:attemptBeginDeliverooJob",function()
    local source=source
    local user_id=RNG.getUserId(source)
    local deliverooJobTable={}
    if RNG.hasGroup(user_id,"Deliveroo")then
        if not usersInDeliverooJob[user_id] then
            ...
            TriggerClientEvent("RNG:beginDeliverooJob",source)
            Wait(100)
            TriggerClientEvent("RNG:deliverooJobSetNextBlip",source,deliverooStops[deliverooJobTable.stopNumber].stopPosition)
        else
            RNGclient.notify(source,{"You are already a Deliveroo Driver."})
        end
    else
        RNGclient.notify(source,{"~r~You are not clocked on as a Deliveroo Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(deliverooStops)do
                    if RNG.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(RNG.getUserSource(k)))-deliverooStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            RNG.giveBankMoney(k,pay)
                            TriggerClientEvent("RNG:phoneNotification", RNG.getUserSource(k), "You received £"..getMoneyStringFormatted(pay), "Deliveroo")
                            TriggerClientEvent("RNG:deliverooJobReachedNextStop",RNG.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#deliverooStops then
                                TriggerClientEvent("RNG:deliverooJobEnd", RNG.getUserSource(k), true)
                                v.currentJob.jobActive=false
                                usersInDeliverooJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("RNG:deliverooJobSetNextBlip",RNG.getUserSource(k),deliverooStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
