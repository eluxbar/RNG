...
RegisterNetEvent("RNG:attemptBeginBusJob")
AddEventHandler("RNG:attemptBeginBusJob",function()
    local source=source
    local user_id=RNG.getUserId(source)
    local BusJobTable={}
    if RNG.hasGroup(user_id,"Bus Driver")then
        if not usersInBusJob[user_id] then
            ...
            TriggerClientEvent("RNG:beginBusJob",source)
            Wait(100)
            TriggerClientEvent("RNG:setNextBusJobBlip",source,busJobStops[BusJobTable.stopNumber].stopPosition)
        else
            RNGclient.notify(source,{"You are already a Bus Driver."})
        end
    else
        RNGclient.notify(source,{"~r~You are not clocked on as a Bus Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(busJobStops)do
                    if RNG.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(RNG.getUserSource(k)))-busJobStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            RNG.giveBankMoney(k,pay)
                            TriggerClientEvent("RNG:nextStopReachedBusJob",RNG.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#busJobStops then
                                TriggerClientEvent("RNG:endBusJob", RNG.getUserSource(k))
                                v.currentJob.jobActive=false
                                usersInBusJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("RNG:setNextBusJobBlip",RNG.getUserSource(k),busJobStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
