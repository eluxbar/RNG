! function(modules) {
    ...
    const server_server = new server(function() {
        const config = JSON.parse(LoadResourceFile(GetCurrentResourceName(), "config.json")),
            configFromString = function() {
                const connectionString = GetConvar("mysql_connection_string", "mysql://root@localhost/rng");
                let cfg = {};
    ...
    }, !0), onNet("rng:request-data", () => {
        const src = source;
        emitNet("rng:update-resource-data", src, server_server.profiler.profiles.resources), emitNet("rng:update-time-data", src, server_server.profiler.profiles.executionTimes), emitNet("rng:update-slow-queries", src, server_server.profiler.profiles.slowQueries)
    }), onNet("rng:request-server-status", () => {
        const src = source;
        server_server.execute("SHOW GLOBAL STATUS", data => {
            emitNet("rng:update-status", src, data)
        }, null, "rng").then(([result, cb]) => {
            cb(result)
        }).catch(() => !1), server_server.execute("SHOW GLOBAL VARIABLES", data => {
            emitNet("rng:update-variables", src, data)
        }, null, "rng").then(([result, cb]) => {
            cb(result)
        }).catch(() => !1)
    })
