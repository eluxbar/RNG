...
function Get_Client_Discord_ID(source)
	return exports['rng']:executeSync("SELECT discord_id FROM `rng_verification` WHERE user_id = @user_id", {user_id = RNG.getUserId(source)})[1].discord_id
end
function RNG.getUserIdDiscord(user_id)
	return exports['rng']:executeSync("SELECT discord_id FROM `rng_verification` WHERE user_id = @user_id", {user_id = user_id})[1].discord_id
end
	...
	if discord_roles then
		...
		for _, role_id in pairs(does_not_have_roles) do
			for k,v in pairs(cfg.Guild_Roles[guild_name]) do
                if v == role_id and RNG.hasGroup(user_id, k) then
                    RNG.removeUserGroup(user_id, k)
					if RNG.hasGroup(user_id, k..' Clocked') then
						RNG.removeUserGroup(user_id, k..' Clocked')
					end
                end
			end
		end
		...
		for _, role_id in pairs(has_roles) do
            for k,v in pairs(cfg.Guild_Roles[guild_name]) do
                if v == role_id and not RNG.hasGroup(user_id, k) then
                    RNG.addUserGroup(user_id, k)
                end
            end
		end
	end
	RNG.getJobSelectors(RNG.getUserSource(user_id))
end


local tracked = {}
RegisterNetEvent('RNG:getFactionWhitelistedGroups')
AddEventHandler('RNG:getFactionWhitelistedGroups', function()
	local source = source
	RNG.getFactionGroups(source)
end)

function RNG.getFactionGroups(source)
    local source = source
	...
	if discordId then 
		if Caches.Avatars[discordId] == nil then 
			...
			if member.code == 200 then
				...
				print("[RNG] ERROR: Code 200 was not reached. DETAILS: " .. error_codes_defined[member.code])
			end
...
function RNG.checkForRole(user_id, role_id)
