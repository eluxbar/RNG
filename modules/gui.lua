...
function RNG.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  RNGclient.openMenuData(source,{menudata})
end

-- force close player menu
function RNG.closeMenu(source)
  RNGclient.closeMenu(source,{})
end
...
function RNG.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result

  RNGclient.prompt(source,{title,default_text})
end
...
function RNG.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  RNGclient.request(source,{id,text,time}) -- send request to client
...
function RNG.registerMenuBuilder(name, builder)
...
function RNG.buildMenu(name, data, cbr)
...
function RNG.openMainMenu(source)
  RNG.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    RNG.openMenu(source,menudata) -- open the generated menu
  end)
end

-- SERVER TUNNEL API

function tRNG.closeMenu(id)
...
function tRNG.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tRNG.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tRNG.requestResult(id,ok)
...
function tRNG.openMainMenu()
  RNG.openMainMenu(source)
end
...
function RNG.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for RNG.addStaticMenuChoices calls
...
AddEventHandler("RNG:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    RNGclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
