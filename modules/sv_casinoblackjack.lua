...
MySQL.createCommand("casinochips/get_rateback", "SELECT rateback FROM rng_casino_chips WHERE user_id = @user_id")
MySQL.createCommand("casinochips/add_rateback", "UPDATE rng_casino_chips SET rateback = (rateback + @amount) WHERE user_id = @user_id")
MySQL.createCommand("casinochips/add_money_lost", "UPDATE rng_casino_chips SET money_lost = (money_lost + @amount) WHERE user_id = @user_id")
MySQL.createCommand("casinochips/add_money_won", "UPDATE rng_casino_chips SET money_won = (money_won + @amount) WHERE user_id = @user_id")
MySQL.createCommand("casinochips/add_losses", "UPDATE rng_casino_chips SET losses = (losses + 1) WHERE user_id = @user_id")
MySQL.createCommand("casinochips/add_total_bets", "UPDATE rng_casino_chips SET total_bets = (total_bets + 1) WHERE user_id = @user_id")
MySQL.createCommand("casinochips/add_wins", "UPDATE rng_casino_chips SET wins = (wins + 1) WHERE user_id = @user_id")
MySQL.createCommand("casinochips/remove_chips", "UPDATE rng_casino_chips SET chips = (chips - @amount) WHERE user_id = @user_id")
...
function BetPlaced(source, total_bets)
    local user_id = RNG.getUserId(source)
    MySQL.execute("casinochips/add_total_bets", {user_id = user_id, total_bets = total_bets})
end

function CasinoWins(source, wins)
    local user_id = RNG.getUserId(source)
    MySQL.execute("casinochips/add_wins", {user_id = user_id, wins = wins})
end

function MoneyLost(source, amount)
    local user_id = RNG.getUserId(source)
    MySQL.execute("casinochips/add_money_lost", {user_id = user_id, amount = amount})
end

function MoneyWon(source, amount)
    local user_id = RNG.getUserId(source)
    MySQL.execute("casinochips/add_money_won", {user_id = user_id, amount = amount})
end

function giveRateback(source, amount, losses)
    local user_id = RNG.getUserId(source)
    MySQL.execute("casinochips/add_rateback", {user_id = user_id, amount = amount})
    MySQL.execute("casinochips/add_losses", {user_id = user_id, losses = losses})
    TriggerClientEvent('RNG:ratebackUpdated', source)
end
    ...
    if source ~= nil then
        ...
        if chairId > 7 then
            if RNG.hasGroup(user_id,"Highroller") then
                ...
                RNGclient.notify(source,{'You need to be a Highroller to sit at this table.'})
                return
            end
        end
        if chairId <= 7 then
                    ...
                    MySQL.query("casinochips/get_chips", {user_id = user_id}, function(rows, affected)
                        ...
                        if chips >= betAmount then
                            if chairId < 8 and betAmount > 100000 then
                                RNGclient.notify(source,{'Maximum bet at this table is £100,000.'})
                                return
                            end
                            ...
                            BetPlaced(source, 1)
                            -- RNG.sendWebhook('blackjack-bet',"RNG Blackjack Logs", "> Player Name: **"..RNG.getPlayerName(source).."**\n> Player TempID: **"..source.."**\n> Player PermID: **"..user_id.."**\n> Amount: **"..getMoneyStringFormatted(tostring(betAmount)).."**")
                        else 
                                        ...
                                        while nextCardCount >= 1 do
                                            ...
                                            if blackjackGameData[gameId][source][2][nextCardCount] == true then --if hit 
                                                ...
                                                if currentHand > 21 then
                                                    ...
                                                    local tenPercent = lostAmount * 0.1
                                                    TriggerClientEvent("blackjack:notify", source, "-" .. getMoneyStringFormatted(tostring(lostAmount)) .. " chips")
                                                    giveRateback(source, tenPercent, 1)
                                                    MoneyLost(source, lostAmount)
                                                    if lostAmount > 10000000 then
                                                        TriggerClientEvent('chatMessage', -1, "^7Diamond Casino |", { 128, 128, 128 }, ""..RNG.getPlayerName(source).." has LOST "..tostring(getMoneyStringFormatted(lostAmount)).." chips!", "alert")
                                                    end                                                                                                      
                                        ...
                                        if currentHand ~= nil then
                                            if currentHand <= 21 then
                                                ...
                                                if dealerHand > 21 then
                                                    giveChips(source, potentialWinAmount, 1)
                                                    MoneyWon(source, potentialWinAmount)
                                                    CasinoWins(source, 1)
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chatMessage', -1, "^7Diamond Casino |", { 124, 252, 0 }, ""..RNG.getPlayerName(source).." has WON "..tostring(getMoneyStringFormatted(potentialWinAmount)).." chips!", "alert")
                                                            end
                                                        end
                                                    end
                                                ...
                                                elseif currentHand == dealerHand then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chatMessage', -1, "^7Diamond Casino |", { 128, 128, 128 }, ""..RNG.getPlayerName(source).." has LOST "..tostring(getMoneyStringFormatted(potentialPushAmount)).." chips!", "alert")
                                                            end
                                                        end
                                                    end
                                                end
