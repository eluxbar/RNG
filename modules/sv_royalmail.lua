...
RegisterNetEvent("RNG:attemptBeginRoyalMailJob")
AddEventHandler("RNG:attemptBeginRoyalMailJob",function()
    local source=source
    local user_id=RNG.getUserId(source)
    local RoyalMailJobTable={}
    if RNG.hasGroup(user_id,"Royal Mail Driver")then
        if not usersInRoyalMailJob[user_id] then
            ...
            TriggerClientEvent("RNG:beginRoyalMailJob",source)
            Wait(100)
            TriggerClientEvent("RNG:royalMailJobSetNextBlip",source,RoyalMailDrops[RoyalMailJobTable.stopNumber].dropPosition)
        else
            RNGclient.notify(source,{"You are already a Royal Mail Driver."})
        end
    else
        RNGclient.notify(source,{"~r~You are not clocked on as a Royal Mail Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(RoyalMailDrops)do
                    if RNG.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(RNG.getUserSource(k)))-RoyalMailDrops[v.currentJob.stopNumber].dropPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            RNG.giveBankMoney(k,pay)
                            TriggerClientEvent("RNG:phoneNotification", RNG.getUserSource(k), 'You have recieved £'..getMoneyStringFormatted(pay), "Royal Mail")
                            TriggerClientEvent("RNG:royalMailReachedNextStop",RNG.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition,pay)
                            if v.currentJob.stopNumber==#RoyalMailDrops then
                                TriggerClientEvent("RNG:royalMailJobEnd", RNG.getUserSource(k))
                                v.currentJob.jobActive=false
                                usersInRoyalMailJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("RNG:royalMailJobSetNextBlip",RNG.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition)
                            end
                        end
                    end
                end
