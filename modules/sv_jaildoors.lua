...
AddEventHandler("RNG:playerSpawn", function(user_id, source, first_spawn)
    if first_spawn then
        TriggerClientEvent('RNG:policeSyncAllDoors', source, doors, doorStates)
    end
end)

RegisterNetEvent("RNG:policeSetDoorState")
AddEventHandler("RNG:policeSetDoorState", function(doorHash, state)
    local source = source
    local user_id = RNG.getUserId(source)
    if RNG.hasPermission(user_id, 'police.armoury') then
        doorStates[doorHash] = state
        TriggerClientEvent('RNG:policeSyncDoor', -1, doorHash, state)
    end
end)

RegisterNetEvent("RNG:enterPoliceSyncDoors")
AddEventHandler("RNG:enterPoliceSyncDoors", function()
    local source = source
    TriggerClientEvent('RNG:gotAllDoorStates', source, doorStates)
end)