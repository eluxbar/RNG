RMenu.Add(
    "policedog",
    "main",
    RageUI.CreateMenu("Dog Support Unit", "Dog Management", tRNG.getRageUIMenuWidth(), tRNG.getRageUIMenuHeight())
)
AddEventHandler(
    "RNG:setPoliceOnDuty",
    function(a)
    ...
    function()
        if globalOnPoliceDuty and tRNG.hasPoliceCallsign() then
            RMenu:Get("frscenemenu", "main"):SetSubtitle(
                "MPD~w~: " .. tRNG.getPoliceCallsign() .. " - " .. tRNG.getPoliceRank() .. " - " .. tRNG.getPoliceName()
            )
        end
        if globalOnPrisonDuty and tRNG.hasHmpCallsign() then
            RMenu:Get("frscenemenu", "main"):SetSubtitle(
                "HMP~w~: " .. tRNG.getHmpCallsign() .. " - " .. tRNG.getHmpRank() .. " - " .. tRNG.getHmpName()
            )
        end
...
RegisterNetEvent(
    "RNG:sendClientRagdoll",
    function()
        SetPedToRagdoll(tRNG.getPlayerPed(), 12000, 12000, 0, 0, 0, 0)
...
local m = {breed = 1}
local function n()
    local o = GetResourceKvpString("rng_policedog_customisations")
...
local function r(s)
    ...
    local q = k[m.breed]
    p[q] = s
    SetResourceKvp("rng_policedog_customisations", json.encode(p))
end
RageUI.CreateWhile(
    1.0,
    true,
                ...
                function()
                    ...
                    if not f.active then
                        ...
                        if not f.insideVehicle then
                            ...
                            RageUI.Button(
                                "Search Nearby",
                                "Dog will follow and sniff close pedestrians",
                                {},
                                true,
                                function(t, u, v)
                                    if v then
...
function tpDog()
    local z = GetEntityCoords(tRNG.getPlayerPed(), true)
                                        ...
                                        if M < 2.0 then
                                            ...
                                            for O, P in pairs(N) do
                                                if I == GetPlayerPed(P) then
                                                    ...
                                                    TriggerServerEvent(
                                                        "RNG:performDogLog",
                                                        "Instructed dog to attack ID: " .. Q
                                                    )
                                                    TriggerServerEvent("RNG:serverDogAttack", Q)
                                                    H = true
                                                    Wait(1000)
    ...
    while f.currentAction == d.SearchNearby and DoesEntityExist(f.handle) do
        ...
        for W, X in pairs(GetActivePlayers()) do
            ...
            if GetVehiclePedIsUsing(Y) == 0 and IsEntityVisible(Y) and #(GetEntityCoords(Y, true) - L) < 5.0 then
                ...
                local _ = tRNG.clientGetUserIdFromSource(Z)
                if tRNG.getJobType(_) == "" then
        ...
        if #V > 0 then
            ...
            TriggerServerEvent("RNG:policeDogSniffPlayer", a0.source)
        end
        Citizen.Wait(5000)
    end
end
function putInVehicle()
    local a2 = tRNG.getNearestVehicle(7.0)
    Citizen.Trace(a2)
...
function removeFromVehicle()
    if IsPedInAnyVehicle(tRNG.getPlayerPed(), true) then
...
function newDog(q, a4)
    ...
    if not IsPedInAnyPoliceVehicle(a5) then
        ...
        if DoesEntityExist(f.handle) then
            ...
            TriggerServerEvent("RNG:performDogLog", "Created a new police dog")
        end
    end
end
function deleteDog()
    ...
    f.active = false
    f.handle = 0
    TriggerServerEvent("RNG:performDogLog", "Deleted dog")
end
function dogFollow()
    if not f.active then
        return
    ...
    if f.currentAction ~= d.Follow then
        ClearPedTasks(f.handle)
        TaskFollowToOffsetOfEntity(f.handle, tRNG.getPlayerPed(), 0.0, 0.0, 0.0, 7.0, -1, 10.0, true)
        f.currentAction = d.Follow
    else
        ClearPedTasks(f.handle)
        f.currentAction = d.Follow
