local a = module("rng-weapons", "cfg/cfg_weaponsonback")
local b = module("rng-weapons", "cfg/weapons")
    ...
    function()
        for c, d in pairs(b.weapons) do
            if not a.weapons[d.hash] then
                ...
                elseif d.class == "Heavy" or d.class == "mosin" then
                    a.weapons[d.hash] = {
                        bone = 24818,
                        offset = vector3(-0.12, -0.12, -0.13),
                        rotation = vector3(100.0, -3.0, 5.0),
                        model = GetHashKey(d.model),
                        type = "Heavy" and "mosin"
                    }
...
local function g()
    ...
    local i = tRNG.getWeapons()
    local j = false
    local k = globalOnPoliceDuty and tRNG.getPlayerVehicle() ~= 0
    for l in pairs(i) do
        local m = GetHashKey(l)
        local n = a.weapons[m]
        if n then
            local o =
                (not k or n.type ~= "Heavy") and (tRNG.isEmergencyService() or not isInGreenzone) and
                not tRNG.isPlayerInAnimalForm()
