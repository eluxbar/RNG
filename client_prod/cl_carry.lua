...
RegisterCommand(
    ...
    function(f, g)
        if IsPedInAnyVehicle(PlayerPedId(), true) then
            ...
            if not globalPlayerInPrisonZone and not inOrganHeist or tRNG.isStaffedOn() or tRNG.isDev() then
                if GetEntityHealth(tRNG.getPlayerPed()) > 102 then
                    local h = GetEntityCoords(tRNG.getPlayerPed())
                    distanceToCasino = #(h - d)
                    if not carryingBackInProgress and (distanceToCasino > 200 or tRNG.isStaffedOn() or tRNG.isDev()) then
                        local i = tRNG.getPlayerPed()
                        local j = GetClosestPlayer(3)
                        if j ~= -1 then
                            target = GetPlayerServerId(j)
                            if GetEntityHealth(GetPlayerPed(j)) ~= 0 then
                                if not tRNG.isStaffedOn() and not globalLFBOnDuty and not tRNG.isDev() then
                                    TriggerServerEvent("CarryPeople:requestCarry", target)
                                else
                                    TriggerServerEvent(
                                        "CarryPeople:sync",
                                        GetPlayerServerId(tRNG.getPlayerId()),
                                        target
                                    )
                                end
                            else
                ...
                tRNG.notify("~r~You cannot carry in the prison or the organ heist.")
            end
        end
    end,
    false
...
RegisterNetEvent(
    ...
    function()
        tRNG.notify("Someone is trying to carry you, press (~y~Y~w~) to accept (~r~L~w~) to refuse")
        e = true
        Citizen.CreateThread(
            function()
                while e do
                    if IsControlJustPressed(0, 246) then
                        tRNG.notify("~g~Request Accepted")
                        ExecuteCommand("carryaccept")
                        e = false
                    elseif IsControlJustPressed(0, 182) then
                        tRNG.notify("~r~Request Refused")
    ...
    function()
        while true do
            ...
            if carryingBackInProgress then
                ...
                if not tRNG.isStaffedOn() and IsPedFalling(l) and tRNG.isDev() then
                    TriggerServerEvent("CarryPeople:stop", target)
                    carryingBackInProgress = false
                    ClearPedTasks(l)
                    StopAnimTask(l, "missfinale_c2mcs_1", "fin_c2_mcs_1_camman", 0)
...
RegisterNetEvent(
    ...
    function(target, o, p, q, r, s, t, u, v)
        ...
        AttachEntityToEntity(tRNG.getPlayerPed(), w, 0, r, q, s, 0.5, 0.5, u, false, false, false, false, 2, false)
        if v == nil then
            v = 0
        end
...
RegisterNetEvent(
    ...
    function()
        carryingBackInProgress = false
        ClearPedSecondaryTask(tRNG.getPlayerPed())
        DetachEntity(tRNG.getPlayerPed(), true, false)
    end
)
function func_carryAnimation()
    if carryingBackInProgress then
        DisablePlayerFiring(tRNG.getPlayerId(), true)
        if not IsEntityPlayingAnim(tRNG.getPlayerPed(), b, a, 3) then
            TaskPlayAnim(tRNG.getPlayerPed(), b, a, 8.0, -8.0, 100000, c, 0, false, false, false)
        end
    end
end
tRNG.createThreadOnTick(func_carryAnimation)
