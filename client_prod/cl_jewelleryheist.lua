...
RegisterNetEvent(
    "RNG:jewelrySyncSetupReady",
    function(v)
        c = v
    end
)
RegisterNetEvent(
    "RNG:jewelryCreateDevicePickup",
    function(w)
        f =
            tRNG.addMarker(
            w.coords.x,
        ...
        ontick_collectDevice = function()
            if IsControlJustPressed(0, 38) and not d then
                ...
                SetEntityHeading(tRNG.getPlayerPed(), w.h)
                local E =
                ...
                Citizen.CreateThread(
                    function()
                        Wait(20000)
                        TriggerServerEvent("RNG:jewelryCollectDevice")
                        FreezeEntityPosition(tRNG.getPlayerPed(), false)
                        ClearPedTasks(tRNG.getPlayerPed())
                        tRNG.teleport(w.coords.x, w.coords.y, w.coords.z)
                        d = false
                        RemoveAnimDict(x)
                    end
                )
        ...
        SetTimeout(
            600000,
            function()
                tRNG.removeArea("jewelry_collect_device")
                tRNG.removeBlip(e)
            end
        )
    end
...
RegisterCommand(
    ...
    function()
        if tRNG.getUserId() == -1 then
            TriggerEvent("RNG:jewelryStartDoorHackSf")
        end
    end
)
RegisterCommand(
    ...
    function()
        if tRNG.getUserId() == -1 then
            TriggerEvent("RNG:jewelryStartComputerHackSf")
        end
    end
)
RegisterNetEvent(
    "RNG:jewelryStartDoorHackSf",
    function()
        Citizen.CreateThread(
            function()
                Wait(2500)
        ...
        TriggerEvent(
            ...
            function(K, L)
                if K then
                    tRNG.notify("~g~Succesfully hacked!")
                    TriggerServerEvent("RNG:jewelryDoorHackSuccess")
                else
                    tRNG.notify("~r~Failed. Reason: " .. L)
                end
                m = false
            end
        )
...
RegisterNetEvent(
    "RNG:jewelryStartComputerHackSf",
    function()
        Citizen.CreateThread(
            function()
                Wait(2500)
        ...
        startDataCrack(
            ...
            function(K)
                if K then
                    TriggerServerEvent("RNG:jewelryComputerHackSuccess")
                else
                    TriggerServerEvent("RNG:jewelryComputerHackFailed")
                end
                m = false
            end
        )
...
RegisterNetEvent(
    ...
    function(T)
        if T then
            ...
            ontick_hackJewelryDoor = function()
                if not globalOnPoliceDuty then
                    if IsControlJustPressed(0, 38) then
                        TriggerServerEvent("RNG:jewelryHackDoor")
                    end
                end
            end
            tRNG.createArea(
            ...
            tRNG.removeArea("jewelry_hack_door")
            tRNG.removeMarker(h)
        end
    end
)
RegisterNetEvent(
    "RNG:jewelryComputerHackArea",
    function(U)
        if U then
            i =
                tRNG.addMarker(
            ...
            ontick_hackJewelryComputer = function()
                if not globalOnPoliceDuty then
                    if IsControlJustPressed(0, 38) then
                        TriggerServerEvent("RNG:jewelryHackComputer")
                    end
                end
            end
            tRNG.createArea(
            ...
            tRNG.removeArea("jewelry_hack_computer")
            tRNG.removeMarker(i)
        end
    end
)
RegisterNetEvent(
    "RNG:jewelrySyncLootAreas",
    function(V, U)
        if U then
            createJewelryArea(V)
        else
            tRNG.removeArea("break_glass_" .. V)
            tRNG.removeMarker(j[V])
        end
    end
)
RegisterNetEvent(
    "RNG:jewelrySyncModelSwaps",
    function(V)
        if #(GetEntityCoords(tRNG.getPlayerPed()) - a.jewelryCases[V].coords) <= 350 then
            local W =
    ...
    if not j[V] then
        ...
        ontick_breakJewelryCase = function(X)
            if IsControlJustPressed(0, 38) and not l and GetEntityHealth(PlayerPedId()) > 102 then
                if not globalOnPoliceDuty then
                    if GetSelectedPedWeapon(tRNG.getPlayerPed()) ~= GetHashKey("WEAPON_UNARMED") then
                        ...
                        if W ~= 0 then
                            ...
                            StartParticleFxNonLoopedOnEntity(
                                "scr_jewel_cab_smash",
                                GetCurrentPedWeaponEntityIndex(tRNG.getPlayerPed()),
                                0.0,
                                0.0,
                                0.0,
                                0.0,
                            ...
                            SetEntityHeading(tRNG.getPlayerPed(), a.jewelryCases[V].heading)
                            SetModelAsNoLongerNeeded(newModel)
                        ...
                        if X.modelHash == GetHashKey("des_jewel_cab4_start") then
                            ...
                            a0(tRNG.getPlayerPed(), tRNG.getPlayerCoords(), true)
                        else
                            TaskPlayAnim(
                                tRNG.getPlayerPed(),
                                "missheist_jewel",
                            ...
                            a0(tRNG.getPlayerPed(), tRNG.getPlayerCoords(), false)
                        end
                        FreezeEntityPosition(tRNG.getPlayerPed(), false)
                        TriggerServerEvent("RNG:jewelryGrabLoot", X.caseId)
                        Citizen.Wait(1000)
                        SetModelAsNoLongerNeeded("prop_jewel_04a")
                        RemoveAnimDict("missheist_jewel")
                        l = false
                    else
                        tRNG.notify("~r~You must be holding a weapon to smash the glass!")
                    end
                else
                    TriggerServerEvent("RNG:jewelryPoliceSeizeLoot", X.caseId)
                    tRNG.removeArea("break_glass_" .. X.caseId)
                end
            end
        end
        j[V] =
    ...
    function(a3, a4)
        if a4 then
            ...
            ontick_jewelryTeleporterEnter = function()
                if IsControlJustPressed(0, 38) then
                    SetEntityHeading(tRNG.getPlayerPed(), 217.38)
                    tRNG.teleport(a.exitTeleporterCoords.x, a.exitTeleporterCoords.y, a.exitTeleporterCoords.z)
                end
            end
            tRNG.addBlip(
            ...
            ontick_jewelryTeleporterExit = function()
                if IsControlJustPressed(0, 38) then
                    SetEntityHeading(tRNG.getPlayerPed(), 217.38)
                    tRNG.teleport(a.enterTeleporterCoords.x, a.enterTeleporterCoords.y, a.enterTeleporterCoords.z)
                end
            end
            tRNG.addMarker(
        ...
        if a4 then
            ...
            ontick_aiMissionTeleporterEnter = function()
                if IsControlJustPressed(0, 38) then
                    if c then
                        ...
                        b = true
                        Citizen.Wait(1000)
                        TriggerServerEvent("RNG:jewelrySetupHeistStart")
                    else
                        tRNG.notify("~r~You cannot enter right now.")
                    end
                end
            end
            tRNG.addBlip(
            ...
            ontick_aiMissionTeleporterExit = function()
                if IsControlJustPressed(0, 38) then
                    ...
                    b = false
                    TriggerServerEvent("RNG:jewelrySetupHeistLeave")
                end
            end
            tRNG.addMarker(
                a.aiMissionTeleporterExit.x,
...
AddEventHandler(
    ...
    function(q)
        if q == "killed" then
            if b then
                TriggerServerEvent("RNG:jewelrySetupHeistLeave")
                TriggerEvent("RNG:jewelryRemoveDeviceArea")
                b = false
            end
        end
    end
...
RegisterNetEvent(
    "RNG:jewelryMakePedsAttack",
    function(a5)
        while not NetworkDoesEntityExistWithNetworkId(a5) do
            Citizen.Wait(0)
        end
        local y = tRNG.getObjectId(a5)
        ...
        TaskCombatPed(y, tRNG.getPlayerPed(), 0, 0)
        SetPedAccuracy(y, 30)
    end
)
function coolCameraThingOne(W, _, a6)
    ...
    PointCamAtCoord(a7, tRNG.getPlayerCoords().x, tRNG.getPlayerCoords().y, tRNG.getPlayerCoords().z)
...
RegisterNetEvent(
    "RNG:jewelryAlarmTriggered",
    function()
        tRNG.announceMpSmallMsg("ALERT", "An alarm has been triggered at the jewelry store", 9, 10000)
    end
)