...
Citizen.CreateThread(
    function()
        Wait(1000)
        TriggerServerEvent("RNG:requestCoinflipTableData")
        for q = 0, #c.coinflipTables do
            local D = c.coinflipTables[q]
            local E = tRNG.loadModel(D.tableModel)
        ...
        while true do
            if d then
                ...
                if
                 ...
                 then
                    local L = tRNG.loadAnimDict("anim_casino_b@amb@casino@games@shared@player@")
                    TaskPlayAnim(tRNG.getPlayerPed(), L, "sit_exit_left", 1.0, 1.0, 2500, 0, 0.0, false, false, false)
                    RemoveAnimDict("anim_casino_b@amb@casino@games@shared@player@")
                    d = false
                    e = true
                    showHowToCoinflip(false)
                    TriggerServerEvent("RNG:cancelCoinflip", true)
                    TriggerServerEvent("RNG:leaveCoinflipTable")
                    h = {}
                    m = false
                    SetTimeout(
                        5000,
            ...
            if not d then
                if a ~= -1 and b < 2 then
                    if IsControlJustPressed(0, 38) then
                        if f[a] == false then
                            TriggerServerEvent("RNG:requestSitAtCoinflipTable", a)
                        end
                    end
                end
            end
...
function goToCoinflipSeat(o)
    ...
    TaskGoStraightToCoord(tRNG.getPlayerPed(), S.x, S.y, S.z, 1.0, 5000, T.z, 0.01)
    Wait(500)
    local U = v(o)
    local V = A(o)
                    ...
                    function(Y, Z, _)
                        if _ then
                            ...
                            if tonumber(a0) then
                                a0 = tonumber(a0)
                                if a0 > 0 then
                                    TriggerServerEvent("RNG:proposeCoinflip", a0)
                                end
                            end
                        end
                    end
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(Y, Z, _)
                                if _ then
                                    ...
                                    if M.user_id == tRNG.getUserId() then
                                        k = true
                                    else
                                        k = false
                                    end
            ...
            function()
                ...
                if k then
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(Y, Z, _)
                            if _ then
                                TriggerServerEvent("RNG:cancelCoinflip")
                            end
                        end,
                        RMenu:Get("frcoinflip", "mainmenu")
                    )
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(Y, Z, _)
                            if _ then
                                TriggerServerEvent("RNG:acceptCoinflip", i)
                            end
                        end,
                        RMenu:Get("frcoinflip", "mainmenu")
                    )
...
RegisterNetEvent(
    "RNG:addCoinflipProposal",
    function(a1, M)
        h[a1] = M
    end
)
RegisterNetEvent(
    "RNG:cancelCoinflipBet",
    function(a1)
        h[a1] = nil
    end
)
RegisterNetEvent(
    "RNG:clearCoinflipBets",
    function(a1)
        h = {}
    end
)
RegisterNetEvent("RNG:coinflipOutcome",function(a4, a5)
        l = true
        ...
        SetTimeout(10000,function()
                a7 = true
                if a4 then
                    tRNG.notify("~g~ You won £" .. getMoneyStringFormatted(a5.amount) .. "!")
                    SendNUIMessage({transactionType = "playCasinoWin"})
                else
                    tRNG.notify("~r~ You lost £" .. getMoneyStringFormatted(a5.amount) .. "!")
