...
RegisterNetEvent(
    "RNG:addEmergencyCall",
    function(o, p, q, r, s, t)
        local u = 0
        if t == "admin" and tRNG.getStaffLevel() > 0 then
            tRNG.notify("~b~Admin ticket received.")
            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
            local v = #(r - GetEntityCoords(tRNG.getPlayerPed()))
            table.insert(d, {o, p, q, v, s, t, u})
            j()
            e[o] = r
        elseif t == "met" and tRNG.globalOnPoliceDuty() then
            tRNG.notify("~b~MET Police call received.")
            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
            local v = #(r - GetEntityCoords(tRNG.getPlayerPed()))
            table.insert(d, {o, p, q, v, s, t, u})
            j()
            e[o] = r
        elseif t == "nhs" and tRNG.globalNHSOnDuty() then
            tRNG.notify("~b~NHS call received.")
            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
            local v = #(r - GetEntityCoords(tRNG.getPlayerPed()))
            table.insert(d, {o, p, q, v, s, t, u})
            j()
            ...
            if a then
                    ...
                    for A, n in ipairs(f[b]) do
                        ...
                        if t == "admin" then
                            DrawAdvancedText(
                                ...
                                0.0028,
                                0.36,
                                q .. " - Distance " .. math.floor(v) .. "m - " .. u .. " mins ago",
                                255,
                                20,
                                10,
                                255,
                ...
                if CursorInArea(0.4234, 0.4614, 0.9444, 0.9759) then
                    ...
                    if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                        PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                        if h ~= nil then
                            TriggerServerEvent("RNG:TakeTicket", h)
                            a = not a
                            ...
                            if C ~= nil then
                                TriggerServerEvent("RNG:TakeTicket", C)
                                a = not a
                                SetNewWaypoint(e[h].x, e[h].y)
                                TriggerEvent("RNG:showHUD", true)
                                globalHideEmergencyCallUI = false
                                ...
                                tRNG.notify("~r~No calls available.")
                            end
                        end
                    end
                else
...
local function u()
    if IsUsingKeyboard(2) and (tRNG.getStaffLevel() > 0 or globalOnPoliceDuty or globalNHSOnDuty or globalOnPrisonDuty) then
        a = not a
        if not a then
            TriggerEvent("RNG:showHUD", true)
            globalHideEmergencyCallUI = false
