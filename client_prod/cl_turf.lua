...
local i = {
    ...
    {position = vector3(-1105.6610107422, 4940.9853515625, 222.5), commissionRate = 0},
...
Citizen.CreateThread(
    function()
        local j = tRNG.addBlip(-552.90362548828, 5326.1733398438, 72.599586486816, 403, 3, "Meth Trader")
        local k = tRNG.addBlip(133.73462677002, -1305.0340576172, 29.268741607666, 403, 4, "Cocaine Trader")
        local l = tRNG.addBlip(103.47916412354, -1937.1091308594, 19.803705215454, 140, 2, "Weed Trader")
        local m = tRNG.addBlip(3569.8334960938, 3662.5893554688, 33.920593261718, 586, 1, "Heroin Trader")
        local n = tRNG.addBlip(-1107.2316894531, 4944.02734375, 222.70597839355, 150, 1, "Large Arms Trader")
        local o = tRNG.addBlip(5065.6201171875, -4591.3857421875, 2.8652405738831, 150, 1, "Large Arms Trader")
        local r = {
            {
                title = "Weed Trader",
                type = "radius",
...
AddEventHandler(
    ...
    function(v, w)
        if w then
            TriggerServerEvent("RNG:refreshTurfOwnershipData")
            TriggerServerEvent("RNG:requestDrugPriceUpdate")
        end
    end
)
RegisterNetEvent(
    "RNG:attackGangCapture",
    function(x, y)
        f = true
        a = y
...
RegisterNetEvent(
    "RNG:turfDefenseInterrupt",
    function()
        e = false
    end
)
RegisterNetEvent(
    "RNG:initiateGangCaptureCheck",
    function(y, B)
        b[y] = B
    end
)
RegisterNetEvent(
    "RNG:gotTurfOwnershipData",
    function(C)
...
RegisterNetEvent(
    "RNG:setTurfDataProfit",
    function(x, D)
        turfData[x].profit = math.floor(D)
    end
)
function tRNG.isPlayerInTurf()
    for D, E in pairs(h) do
        if #(tRNG.getPlayerCoords() - E.position) < E.distance then
            return true
        end
    end
    return false
    ...
    function(v, w)
        if w then
            ...
            local O = function(M)
                if IsControlJustPressed(1, 51) then
                    if not PlayerIsInGang then
                        tRNG.notify("~r~You need to be in a gang to capture a turf!")
                    elseif not f then
                        TriggerServerEvent("RNG:refreshTurfOwnershipData")
                        TriggerServerEvent("RNG:checkTurfCapture", M.turfId)
                    end
                    Wait(10000)
                end
            end
            for F, P in pairs(h) do
                tRNG.createArea("turf_" .. F, P.position, 1.5, 6, L, N, O, {turfId = F})
            end
            local Q = function(M)
                            ...
                            if T ~= nil and tonumber(T) then
                                ...
                                if T <= 25 and T >= 0 then
                                    ...
                                    if GetGameTimer() > K + 30000 then
                                        ...
                                        if F == 1 then
                                            TriggerServerEvent("RNG:setNewWeedPrice", T)
                                        elseif F == 2 then
                                            TriggerServerEvent("RNG:setNewCocainePrice", T)
                                        elseif F == 3 then
                                            TriggerServerEvent("RNG:setNewMethPrice", T)
                                        elseif F == 4 then
                                            TriggerServerEvent("RNG:setNewHeroinPrice", T)
                                        elseif F == 5 then
                                            TriggerServerEvent("RNG:setNewLargeArmsCommission", T)
                                        elseif F == 6 then
                                            TriggerServerEvent("RNG:setNewLSDNorthPrice", T)
                                        elseif F == 7 then
                                            TriggerServerEvent("RNG:setNewLSDSouthPrice", T)
                                        end
                                    else
                                        tRNG.notify("~r~Rate limit, please wait 30s to change the commission.")
                                    end
                                end
                            end
                        end
            ...
            for F, G in pairs(i) do
                tRNG.createArea("turfmanager_" .. F, G.position, 1.5, 6, Q, R, S, {turfId = F, draw = false})
            end
        end
    end
)
RegisterNetEvent(
    "RNG:captureOwnershipReturned",
    function(F, U, V)
        if not U then
            startGangCapture(F, h[F].position, V)
        else
            tRNG.notify("~r~Turf already owned!")
        end
    end
)
function otherMembersStartGangCapture(x, y, W)
    if b[y] then
        ...
        Citizen.CreateThread(
            function()
                local tmpMarker = tRNG.addMarker(W.x, W.y, W.z - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
                while f do
                    local X = GetEntityCoords(tRNG.getPlayerPed(), false)
                    local Y = #(X - W)
                    if Y >= 37.5 then
                        tRNG.notify("~r~Turf capture cancelled, you got too far!")
                        f = false
                        TriggerServerEvent("RNG:gangDefenseLocationUpdate", y, "Attackers", true)
                    elseif GetEntityHealth(tRNG.getPlayerPed()) <= 102 then
                        tRNG.notify("~r~Turf capture failed, you died")
                        f = false
                        TriggerServerEvent("RNG:gangDefenseLocationUpdate", y, "Attackers", true)
                    end
                    Wait(1000)
                end
                tRNG.removeMarker(tmpMarker)
            end
        )
        Citizen.CreateThread(
            function()
...
function startGangCapture(x, W, y)
    if turfData[x].owned then
        tRNG.notify("~r~You already own this turf!")
        TriggerServerEvent("RNG:failCaptureTurfOwned", x)
    else
        TriggerServerEvent("RNG:initiateGangCapture", x, y)
        local timer = g
        while b[y] == nil do
            Wait(0)
        end
        if b[y] then
            ...
            Citizen.CreateThread(
                function()
                    tmpMarker = tRNG.addMarker(W.x, W.y, W.z - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
                    while f do
                        local X = GetEntityCoords(tRNG.getPlayerPed(), false)
                        local Y = #(X - W)
                        if Y >= 37.5 then
...
function defendGangCapture(x, a, Z, _, a0)
    ...
    Citizen.CreateThread(
        function()
            local tmpMarker = tRNG.addMarker(Z, _, a0 - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
            while e do
                local X = GetEntityCoords(tRNG.getPlayerPed(), false)
                local Y = #(X - vector3(Z, _, a0))
                if Y < 37.5 and GetEntityHealth(tRNG.getPlayerPed()) > 102 then
                    TriggerServerEvent("RNG:gangDefenseLocationUpdate", a, "Defenders", true)
                elseif GetEntityHealth(tRNG.getPlayerPed()) > 102 then
                    TriggerServerEvent("RNG:gangDefenseLocationUpdate", a, "Defenders", false)
                elseif GetEntityHealth(tRNG.getPlayerPed()) <= 102 then
                    tRNG.notify("~r~Turf defense failed, you died")
                    e = false
                    TriggerServerEvent("RNG:gangDefenseLocationUpdate", a, "Defenders", false)
                else
                    TriggerServerEvent("RNG:gangDefenseLocationUpdate", a, "Defenders", false)
...
function gangCaptureSuccess(x, a)
    ...
    TriggerServerEvent("RNG:gangCaptureSuccess", a)
end
function gangDefenseSuccess()
    TriggerServerEvent("RNG:gangDefenseSuccess", a)
end
