            ...
            RageUI.DrawContent(
                ...
                function()
                    ...
                    RageUI.Button(
                        ...
                        function(p, p, u)
                            if u then
                                TriggerServerEvent("RNG:claimNextLoginReward")
                                b = b + 1
...
RageUI.CreateWhile(
    ...
    true,
    function()
        if RageUI.Visible(RMenu:Get("hourrewards", "mainmenu")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = false},
                function()
                    RageUI.Button(
...
RegisterNetEvent(
    "RNG:setDailyRewardInfo",
    function(w, x, y)
        b = w
        c = GetGameTimer() + (86400 - (y - x)) * 1000
        if w == 0 then
            Citizen.Wait(20000)
            tRNG.notify("~y~You have not started your daily reward. Do /dailyrewards to start now!")
        end
    end
)
RegisterCommand(
...
RegisterCommand(
    "hourlyrewards",
    function()
        RageUI.Visible(RMenu:Get("hourrewards", "mainmenu"), true)
    end,
    false
)
Citizen.CreateThread(
    function()
        ...
        while true do
            Citizen.Wait(100)
            if r() and b > 0 then
                tRNG.notify("~g~You can claim your next daily reward at /dailyrewards!")
                break
            end
            Citizen.Wait(6000)
        end
