local a = {
    ...
    [74] = {
        id = 74,
        position = vector3(-2171.619141, 5140.971191, 2.819996),
        name = "VIP Island",
        destination = {75}
    },
    [75] = {
...
RMenu.Add(
    "vkint",
    "mainmenu",
    RageUI.CreateMenu("Teleporter", "Main Menu", tRNG.getRageUIMenuWidth(), tRNG.getRageUIMenuHeight())
)
local b = nil
local function c(d)
    if tRNG.getPlayerCombatTimer() > 0 then
        tRNG.notify("You can not teleport whilst you have a combat timer.")
        return
    end
...
Citizen.CreateThread(
    function()
        for f, d in pairs(a) do
            tRNG.addMarker(d.position.x, d.position.y, d.position.z - 1, 1.0, 1.0, 1.0, 255, 255, 255, 170, 50, 27)
            tRNG.createArea(
                "vkint_" .. d.id,
                d.position,
                2.0,
