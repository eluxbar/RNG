    ...
    function()
        while true do
            ...
            if IsPlayerPlaying(PlayerId()) and a then
                if json.encode(b) ~= json.encode(tRNG.getWeapons()) then
                    b = tRNG.getWeapons()
                    TriggerServerEvent("RNG:updateWeapons", b, decor)
                end
            end
        end
    end
...
Citizen.CreateThread(
    function()
        print("[RNG] Loading User Data.")
        c = json.decode(GetResourceKvpString("rng_userdata") or "{}")
        if type(c) ~= "table" then
            c = {}
            print("[RNG] Loading User Data - Failed To Load Setting To Default.")
        else
            print("[RNG] Loaded User Data")
        end
    end
)
function tRNG.updateCustomization(d)
    local e = tRNG.getCustomization()
    if e.modelhash ~= 0 and IsModelValid(e.modelhash) then
        c.customisation = e
        if d then
            SetResourceKvp("rng_userdata", json.encode(c))
        end
    end
end
function tRNG.updateHealth(d)
    c.health = GetEntityHealth(PlayerPedId())
    if d then
        SetResourceKvp("rng_userdata", json.encode(c))
    end
end
function tRNG.updateArmour(d)
...
Citizen.CreateThread(
    function()
        ...
        while true do
            Wait(5000)
            if not tRNG.isInHouse() and not inOrganHeist and not tRNG.isPlayerInRedZone() and not tRNG.isInSpectate() then
                tRNG.updatePos()
            end
            if not tRNG.isStaffedOn() and not tRNG.isPlayerInAnimalForm() then
                tRNG.updateCustomization()
            end
            tRNG.updateHealth()
            tRNG.updateArmour()
            SetResourceKvp("rng_userdata", json.encode(c))
...
function tRNG.checkCustomization()
    local e = c.customisation
    if e == nil or e.modelhash == 0 or not IsModelValid(e.modelhash) then
        tRNG.setCustomization(getDefaultCustomization(), true, true)
    else
        tRNG.setCustomization(e, true, true)
    end
end
...
function tRNG.spawnAnim()
    DoScreenFadeOut(250)
    tRNG.hideUI()
    local k = c.position or vector3(-56.818600, -1110.795166, 26.435785)
    Wait(500)
    ...
    TriggerEvent("RNG:ForceRefreshData", -1)
    DoScreenFadeIn(1000)
    SetBigmapActive(false)
    ShutdownLoadingScreenNui()
    if not tRNG.isDev() then
...
RegisterNetEvent("RNG:setHairstyle")
AddEventHandler(
    "RNG:setHairstyle",
    function(o)
        if o then
