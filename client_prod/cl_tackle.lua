...
function tRNG.isPedBeingTackled()
    return h
end
RegisterNetEvent(
    "RNG:playTackle",
    ...
    for p, q in ipairs(GetActivePlayers()) do
        if q ~= PlayerId() then
            ...
            if t ~= "metpd" and t ~= "hmp" and t ~= "npas" then
                ...
                local v = #(u - tRNG.getPlayerCoords())
                if v < n then
                    n = v
                    o = r
                end
...
function func_tackleManagement()
    if h then
        SetPedToRagdoll(tRNG.getPlayerPed(), 1000, 1000, 0, 0, 0, 0)
    end
    if globalOnPoliceDuty or w() or tRNG.isStaffedOn() or globalOnPrisonDuty and globalPlayerInPrisonZone then
        if IsControlPressed(0, a["LEFTSHIFT"]) and IsControlPressed(0, a["G"]) then
            if
                not b and GetGameTimer() - g > 10 * 1000 and GetEntityHealth(PlayerPedId()) > 102 and
                    not tRNG.isKnockedOut()
             then
                local x = m()
                if x then
                    if
                        not b and not c and not IsPedInAnyVehicle(tRNG.getPlayerPed()) and
                            not IsPedInAnyVehicle(GetPlayerPed(x))
                     then
                        b = true
                        g = GetGameTimer()
                        TriggerServerEvent("RNG:tryTackle", x)
                    end
                end
            end
        end
...
tRNG.createThreadOnTick(func_tackleManagement)
