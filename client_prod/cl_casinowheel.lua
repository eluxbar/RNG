...
RegisterNetEvent("RNG:frameworkCleanup")
AddEventHandler(
    "RNG:frameworkCleanup",
    function()
        DeleteEntity(b)
    end
)
        ...
        RageUI.IsVisible(
            ...
            function()
                RageUI.ButtonWithStyle(
                    "Spin the wheel of RNG (50,000 Chips)",
                    "",
                    {RightLabel = "→→→"},
                    true,
                    function(e, f, g)
                        if g then
                            TriggerServerEvent("RNG:requestSpinLuckyWheel")
                        end
                    end
                )
            end,
...
Citizen.CreateThread(
    function()
        ...
        local i = tRNG.loadModel("vw_prop_vw_luckywheel_02a")
        local j = tRNG.loadModel(c)
...
function goToWheel()
    if not d then
        ...
        local D = false
        while not D do
            local E = GetEntityCoords(tRNG.getPlayerPed())
        ...
        SetEntityHeading(tRNG.getPlayerPed(), 10.9754)
        Wait(2000)
    ...
    function(D, E)
        if E then
            ...
            for K, L in pairs(a) do
                tRNG.addBlip(L.entryPosition.x, L.entryPosition.y, L.entryPosition.z, 681, 0, "RNG Wheel", 0.7, true)
                tRNG.createArea("frwheel_" .. K, L.entryPosition, 1.5, 6, F, H, I, {})
            end
        end
    end
)
RegisterNetEvent("RNG:syncLuckyWheel")
AddEventHandler(
    "RNG:syncLuckyWheel",
    function(M)
        Wait(900)
...
RegisterNetEvent("RNG:spinLuckyWheel")
AddEventHandler(
    "RNG:spinLuckyWheel",
    function()
        goToWheel()
...
RegisterNetEvent("RNG:spinLuckyWheelReaction")
AddEventHandler(
    "RNG:spinLuckyWheelReaction",
    function(N)
        local y = tRNG.getPlayerPed()
