...
RegisterNetEvent(
    "RNG:prisonBeanbagShotgunClient",
    function(o)
...
AddEventHandler(
    ...
    function(p, q)
        if q then
            ...
            local s = function()
                if b then
                    TriggerServerEvent("RNG:prisonerSentBackToCell")
                    tRNG.teleport(a.prisonCells[d].coords.x, a.prisonCells[d].coords.y, a.prisonCells[d].coords.z)
                    tRNG.notify("Naughty, go back.")
                end
                globalPlayerInPrisonZone = false
            end
            local t = function()
            end
            tRNG.createArea("forceStayInPrison", a.prisonMainCoords, 225.0, 100.0, r, s, t, {})
            local u = function(v)
                    ...
                    if IsControlJustPressed(0, 38) then
                        if not h then
                            if v.type == "startingLocations" then
                                h = true
                                i = v.job
                                TriggerServerEvent("RNG:prisonStartJob", v.job)
                            elseif i == v.job then
                                h = true
                            end
                            if h then
                                        ...
                                        while y do
                                            if v.data.isScenario == nil then
                                                if
                                                 ...
                                                 then
                                                    tRNG.loadAnimDict(v.data.animDict)
                                                    TaskPlayAnim(
                                                        PlayerPedId(),
                                                        v.data.animDict,
                                                        v.data.animName,
                                ...
                                Citizen.CreateThread(
                                    function()
                                        ...
                                        tRNG.notify(v.data.nextAction)
                                        if v.type == "endingLocations" then
                                            i = ""
                                            TriggerServerEvent("RNG:prisonEndJob", v.job)
                                        end
                                        h = false
                                    end
                                )
                ...
                for B, C in pairs(A) do
                    if type(C.coords) == "table" then
                        for D = 1, #C.coords, 1 do
                            ...
                            tRNG.add3DTextForCoord(C.action, C.coords[D].x, C.coords[D].y, C.coords[D].z + 1.0, 2.2)
                        end
                    else
                        tRNG.createArea(
                            "prisonJob_" .. z .. B,
                        ...
                        tRNG.add3DTextForCoord(C.action, C.coords.x, C.coords.y, C.coords.z + 1.0, 2.2)
                    end
                end
            end
        end
...
RegisterNetEvent(
    "RNG:setPrisonGuardOnDuty",
    function(E)
        globalOnPrisonDuty = E
    end
)
RegisterNetEvent(
    "RNG:forcePlayerInPrison",
    function(E)
        b = E
        globalInPrison = E
    end
...
tRNG.createThreadOnTick(func_drawPrisonTimerBar)
RegisterNetEvent(
    "RNG:prisonCreateBreakOutAreas",
    function()
        local H = false
        local I = function()
        ...
        local K = function(v)
            if IsControlJustPressed(0, 38) and not g then
                if tRNG.TriggerServerCallback("RNG:prisonStartWireCutting") then
                    g = true
                    tRNG.loadAnimDict("anim@gangops@facility@servers@")
                    FreezeEntityPosition(PlayerPedId(), true)
                    TaskPlayAnim(
                        PlayerPedId(),
                    ...
                    tRNG.notify("~g~Started wire cutting (Press X to cancel)")
                    Citizen.CreateThread(
                        function()
                            SetTimeout(
                                60000,
                            ...
                            if H then
                                FreezeEntityPosition(PlayerPedId(), false)
                                ClearPedTasks(PlayerPedId())
                                TriggerServerEvent("RNG:prisonBreakOutSuccess")
                                local L = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 2.5, 0.0)
                                tRNG.teleport(L.x, L.y, L.z)
                            end
                        end
                    )
                else
                    tRNG.notify("You do not have the required equipment!")
                end
            end
            if IsControlJustPressed(0, 73) and g then
                ...
                ClearPedTasks(PlayerPedId())
                FreezeEntityPosition(PlayerPedId(), false)
                tRNG.notify("Wire cutting cancelled!")
            end
        end
...
RegisterNetEvent(
    ...
    function(M)
        ...
        local P = function(v)
            if globalInPrison then
                tRNG.DrawText3D(v.coords, "[E] Pickup " .. v.item, 0.5)
                if IsControlJustPressed(0, 38) then
                    TriggerServerEvent("RNG:prisonPickupItem", v.item)
                end
            end
        end
        for z, A in pairs(M) do
            tRNG.removeArea("prisonPickupItem_" .. z)
            tRNG.createArea("prisonPickupItem_" .. z, A.coords, 2.0, 5.0, N, O, P, {item = z, coords = A.coords})
        end
    end
)
RegisterNetEvent(
    "RNG:prisonRemoveItemAreas",
    function(Q)
        tRNG.removeArea("prisonPickupItem_" .. Q)
    end
)
RegisterNetEvent(
    "RNG:prisonReleased",
    function()
        b = false
        globalInPrison = false
        tRNG.notify("~g~You have been released!")
        tRNG.teleport(a.prisonLeaveCoords.x, a.prisonLeaveCoords.y, a.prisonLeaveCoords.z)
        Citizen.Wait(100)
...
local function R()
    ...
    if S == GetHashKey("mp_m_freemode_01") then
        tRNG.loadCustomisationPreset("PrisonerMale")
    else
        tRNG.loadCustomisationPreset("PrisonerFemale")
    end
end
RegisterNetEvent(
    "RNG:putInPrisonOnSpawn",
    function(T)
        Citizen.Wait(5000)
        tRNG.teleport(a.prisonCells[T].coords.x, a.prisonCells[T].coords.y, a.prisonCells[T].coords.z)
        b = true
        globalInPrison = true
        d = T
        Citizen.Wait(100)
        ...
        if S ~= GetHashKey("mp_m_freemode_01") and S ~= GetHashKey("mp_f_freemode_01") then
            tRNG.setCustomization({modelhash = "mp_m_freemode_01"})
        end
        Citizen.Wait(2000)
        R()
    end
...
RegisterNetEvent(
    "RNG:prisonTransportWithBus",
    function(T)
        DetachEntity(PlayerPedId(), false, true)
        R()
        c = true
        local U = getClosestPoliceStation()
        tRNG.teleport(U.coords.x, U.coords.y, U.coords.z)
        Citizen.CreateThread(
            function()
                while c do
        ...
        local V = tRNG.spawnVehicle("pbus", U.coords.x, U.coords.y, U.coords.z, U.heading, false, false, false)
        ...
        enter_prisonArrive = function()
            ...
            while #(tRNG.getPlayerCoords() - a.prisonArriveMainDoor) > 1 do
                Citizen.Wait(0)
            end
            tRNG.teleport(a.prisonCells[T].coords.x, a.prisonCells[T].coords.y, a.prisonCells[T].coords.z)
            Citizen.Wait(100)
            ...
            DeleteEntity(V)
            DeleteEntity(W)
            TriggerServerEvent("RNG:prisonArrivedForJail")
            c = false
            b = true
            globalInPrison = true
            d = T
...
RegisterNetEvent(
    "RNG:prisonSpawnInMedicalBay",
    function()
        Citizen.Wait(2000)
        tRNG.teleport(a.medicalBayBed.x, a.medicalBayBed.y, a.medicalBayBed.z)
        b = true
        Citizen.Wait(100)
        SetEntityHeading(PlayerPedId(), a.medicalBayHeading)
        tRNG.loadAnimDict("mp_bedmid")
...
RageUI.CreateWhile(
    ...
    true,
    function()
        if RageUI.Visible(RMenu:Get("rngPrisonItemMenu", "main")) then
            RageUI.DrawContent(
                    ...
                    for D = 1, #a.craftableItems, 1 do
                        RageUI.ButtonWithStyle(
                            ...
                            function(X, Y, Z)
                                ...
                                if Z then
                                    if tRNG.TriggerServerCallback("RNG:prisonCraftItem", currentButton) then
                                        local _ = true
                                        FreezeEntityPosition(PlayerPedId(), true)
                                        tRNG.loadAnimDict("amb@medic@standing@kneel@base")
                                        TaskPlayAnim(
                                            PlayerPedId(),
                                        ...
                                        tRNG.notify("You do not have the required items to craft this!")
                                    end
                                end
                            end,
                            nil
    ...
    function()
        if globalOnPrisonDuty then
            if n then
                ...
                local a0 = tRNG.TriggerServerCallback("RNG:requestPrisonerData")
                if a0 ~= nil then
                    n = true
...
function createPrisonCraftMenu()
    tRNG.removeMarker(f)
    tRNG.removeArea("prisonCraftItemArea")
    local a2 = function()
        RageUI.Visible(RMenu:Get("rngPrisonItemMenu", "main"), true)
    end
    local a3 = function()
        RageUI.Visible(RMenu:Get("rngPrisonItemMenu", "main"), false)
    end
    local a4 = function()
    end
    local a5 = a.prisonMenuCoords[d].coords
    tRNG.createArea("prisonCraftItemArea", a5, 1.0, 3.0, a2, a3, a4, {})
    f = tRNG.addMarker(a5.x, a5.y, a5.z - 0.975, 0.4, 0.4, 0.5, 30, 50, 255, 255, 30, 27, false, false, false)
end
...
function getClosestPoliceStation()
    for D = 1, #a.allPoliceStations, 1 do
        if #(tRNG.getPlayerCoords() - a.allPoliceStations[D].coords) <= 800 then
...
function nearPrisonPayPhone()
    for D = 1, #a.prisonPayPhones, 1 do
        if #(tRNG.getPlayerCoords() - a.prisonPayPhones[D].coords) <= 3.0 then
            return true
        end
    end
    return false
