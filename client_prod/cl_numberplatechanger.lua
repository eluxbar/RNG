...
local d = module("rng-vehicles", "cfg/cfg_garages")
d = d.garages
RMenu.Add("plateshop", "main", RageUI.CreateMenu("", "DVLA", 1350, 50, "rng_licenseplateui", "rng_licenseplateui"))
RMenu.Add(
    "plateshop",
    "ownedvehicles",
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(e, f, g)
                            if g then
                                ...
                                if h ~= "" then
                                    TriggerServerEvent("RNG:checkPlateAvailability", h)
                                end
                            end
                        end
                    )
            ...
            RageUI.DrawContent(
                ...
                function()
                    RageUI.Button(
                        ...
                        function(e, f, g)
                            if g then
                                TriggerServerEvent("RNG:ChangeNumberPlate", selectedCar)
                            end
                        end
                    )
                end
        ...
        if i then
            ...
            local d = function(k)
                if IsControlJustPressed(1, 38) then
                    TriggerServerEvent("RNG:getCars")
                    RageUI.ActuallyCloseAll()
                    RageUI.Visible(RMenu:Get("plateshop", "main"), not RageUI.Visible(RMenu:Get("plateshop", "main")))
                end
                tRNG.DrawText3D(c, "Press [E] to open License Plate Management", 0.4)
            end
            tRNG.createArea("licenseplate", c, 1.5, 6, j, n, d, {})
            tRNG.addMarker(c.x, c.y, c.z - 1, 1.0, 1.0, 1.0, 255, 0, 0, 170, 50, 27)
            tRNG.addBlip(c.x, c.y, c.z, 606, 2, "Licence Plate Manager")
        end
    end
)
RegisterNetEvent("RNG:RecieveNumberPlate")
AddEventHandler(
    "RNG:RecieveNumberPlate",
    function(p)
        a = p
        RageUI.ActuallyCloseAll()
...
RegisterNetEvent("RNG:carsTable")
AddEventHandler(
    "RNG:carsTable",
    function(q)
        b = q
