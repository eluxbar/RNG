RMenu.Add(
    "mainmenu",
    "cinematic",
    RageUI.CreateMenu("", "", tRNG.getRageUIMenuWidth(), tRNG.getRageUIMenuHeight(), "new_editor", "r_editor_header")
)
RMenu:Get("mainmenu", "cinematic"):SetSubtitle("RNG Cinematic Editor")
RMenu.Add(
    "load_scene",
    "cinematic",
...
Citizen.CreateThread(
    function()
        m = json.decode(GetResourceKvpString("rng_scenedata") or "{}")
    end
)
RegisterNetEvent(
    "RNG:openCinematicMenu",
    function()
        a(true)
    end
)
function tRNG.createCinematicScene(P)
    m[P] = {}
    SetResourceKvp("rng_scenedata", json.encode(m))
    tRNG.notify("~g~Scene created & saved.")
end
function tRNG.deleteCinematicScene(P)
    m[P] = nil
    SetResourceKvp("rng_scenedata", json.encode(m))
    tRNG.notify("~g~Scene deleted & saved.")
end
function tRNG.createCamera(P, Q)
    if #m[P] == 0 then
        Q.transition = 100
    end
    table.insert(m[P], Q)
    ...
    SetResourceKvp("rng_scenedata", json.encode(m))
    tRNG.notify("~g~Camera created & saved.")
end
function tRNG.modifyCamera(P, Q, R)
    m[P][R] = Q
    SetResourceKvp("rng_scenedata", json.encode(m))
    tRNG.notify("~g~Camera modified & saved.")
end
function tRNG.deleteCamera(P, S)
    table.remove(m[P], S)
    SetResourceKvp("rng_scenedata", json.encode(m))
    tRNG.notify("~g~Camera deleted & saved.")
end
function tRNG.modifyTransition(n, q, T)
    m[n][q].transition = tonumber(T)
    SetResourceKvp("rng_scenedata", json.encode(m))
    tRNG.notify("~g~Transition change saved.")
end
function tRNG.modifyCameraBlending(n, q, U)
    m[n][q].blending = l[U]
    SetResourceKvp("rng_scenedata", json.encode(m))
    tRNG.notify("~g~Camera Blending change saved.")
end
function tRNG.modifyScreeneffect(n, q, V)
    m[n][q].screeneffect = V
    SetResourceKvp("rng_scenedata", json.encode(m))
    tRNG.notify("~g~Screeneffect change saved.")
end
function tRNG.modifyTimecycleEffect(n, q, W)
    m[n][q].timecycleEffect = W
    SetResourceKvp("rng_scenedata", json.encode(m))
    tRNG.notify("~g~Timecycle Effect change saved.")
end
function tRNG.setTimecycleIntensity(n, q, f)
    m[n][q].timecycleIntensity = f
    SetResourceKvp("rng_scenedata", json.encode(m))
    tRNG.notify("~g~Timecycle intensity change saved.")
end
function tRNG.modifyShakeEffect(n, q, X)
    m[n][q].shake = X
    SetResourceKvp("rng_scenedata", json.encode(m))
    tRNG.notify("~g~Shake change saved.")
end
function tRNG.modifyCameraType(n, q, type)
    m[n][q].type = type
    SetResourceKvp("rng_scenedata", json.encode(m))
    tRNG.notify("~g~Camera type change saved.")
end
function tRNG.addCameraFocus(n, q, Y, Z)
    m[n][q].focusType = Y
    m[n][q].focusData = Z
    SetResourceKvp("rng_scenedata", json.encode(m))
    tRNG.notify("~g~Follow player change saved.")
end
function tRNG.setCameraFov(n, q)
    m[n][q].fov = i[A]
    SetResourceKvp("rng_scenedata", json.encode(m))
    tRNG.notify("~g~FOV change saved.")
end
                    ...
                    if q and m[n][q] then
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(a1, a2, a3)
                                if a3 then
                                    ...
                                    if tonumber(T) then
                                        tRNG.modifyTransition(n, q, T)
                                    else
                                        tRNG.notify("Failed to change transition time.")
                                    end
                                end
                            end,
                            RMenu:Get("camera_manager", "cinematic")
                        ...
                        RageUI.List(
                            ...
                            function(a1, a2, a3, a8)
                                if a8 ~= x then
                                    x = a8
                                    tRNG.modifyCameraBlending(n, q, k[x])
                                end
                            end,
                            function()
                            end,
                            ...
                            function(a1, a2, a3, a8)
                                ...
                                if a8 ~= e then
                                    ...
                                    if m[n][q].timecycleEffect ~= "" then
                                        ...
                                        tRNG.setTimecycleIntensity(n, q, f[e])
                                    end
                                end
                            end,
                            function()
                    ...
                    for g = 1, #c do
                        RageUI.ButtonWithStyle(
                            ...
                            function(a1, a2, a3)
                                ...
                                if a3 then
                                    tRNG.modifyScreeneffect(n, q, c[g])
                                end
                            end,
                            RMenu:Get("camera_manager", "cinematic")
                        )
                    ...
                    for g = 1, #d do
                        RageUI.ButtonWithStyle(
                            ...
                            function(a1, a2, a3)
                                ...
                                if a3 then
                                    tRNG.modifyTimecycleEffect(n, q, d[g])
                                end
                            end,
                            RMenu:Get("camera_manager", "cinematic")
                        )
                    ...
                    for g = 1, #h do
                        RageUI.ButtonWithStyle(
                            ...
                            function(a1, a2, a3)
                                ...
                                if a3 then
                                    tRNG.modifyShakeEffect(n, q, h[g])
                                end
                            end,
                            RMenu:Get("camera_manager", "cinematic")
                        )
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(a1, a2, a3)
                            if a3 then
                                ...
                                if a9 ~= "" then
                                    tRNG.addCameraFocus(n, q, "player", a9)
                                end
                            end
                        end,
                        RMenu:Get("camera_manager", "cinematic")
                            ...
                            if a3 then
                                tRNG.clientPrompt(
                                    ...
                                    function(aa)
                                        ...
                                        tRNG.addCameraFocus(n, q, "coord", vector3(ad, ae, af))
                                    end
                                )
                            end
                        end,
...
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            if (o or p) and noclipActive and tRNG.getPlayerVehicle() ~= 0 then
                tRNG.notify("You may not noclip when in a vehicle.")
                tRNG.toggleNoclip()
            end
                ...
                if a0.focusType ~= "" then
                    if a0.focusType == "player" then
                        ...
                        if aj then
                            ...
                            print("[RNG Cinematic] Failed to point cam at player, could not get entity.")
                        end
...
Citizen.CreateThread(
    function()
        while true do
            local al, am = tRNG.getPlayerVehicle()
            ...
            if an ~= 0 and IsEntityAPed(an) and IsPedAPlayer(an) and not IsEntityVisible(an) then
                ...
                if ao ~= -1 then
                    ...
                    if ap > 0 then
                        if not tRNG.clientGetPlayerIsStaff(tRNG.clientGetUserIdFromSource(ap)) then
