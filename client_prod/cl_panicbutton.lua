...
RegisterCommand(
    ...
    function()
        if globalOnPoliceDuty or globalNHSOnDuty or globalOnPrisonDuty or globalLFBOnDuty or PlayerIsInGang then
            if b.Cooling == 0 then
                local f = {}
                f.Player = tRNG.getPlayerId()
                f.Ped = tRNG.getPlayerPed()
                f.Name = tRNG.getPlayerName(f.Player)
                ...
                if globalOnPoliceDuty then
                    TriggerServerEvent("RNG:newPanic", f, 3)
                elseif globalNHSOnDuty then
                    TriggerServerEvent("RNG:newPanic", f, 2)
                elseif globalOnPrisonDuty then
                    TriggerServerEvent("RNG:newPanic", f, 4)
                elseif globalLFBOnDuty then
                    TriggerServerEvent("RNG:newPanic", f, 5)
                elseif PlayerIsInGang then
                    TriggerServerEvent("RNG:newGangPanic", f, 6)
                end
...
RegisterNetEvent(
    "RNG:returnPanic",
    function(g, f, h)
        local i = tRNG.getUserId()
        if
            globalOnPoliceDuty or globalNHSOnDuty or globalOnPrisonDuty or globalLFBOnDuty and h == 5 or
                h == 6 and not tRNG.isEmergencyService()
         then
            if h == 6 and tRNG.isEmergencyService() then
                return
            end
