...
local k = j["Red"]
local l = GetResourceKvpString("rng_gang_colour") or "Red"
local function m()
    return RNGGangMembers
end

AddEventHandler("RNG:onClientSpawn",function()
    TriggerEvent("RNG:ForceRefreshData")
    TriggerServerEvent("RNG:GetGangData")
end)

RegisterNetEvent("RNG:GotGangData")
AddEventHandler("RNG:GotGangData",function(n, o, h, p, q, r, s)
    if n == nil then
        PlayerIsInGang = false
        tRNG.setFriendlyFire(false)
    else
        PlayerIsInGang = true
        tRNG.setFriendlyFire(true)
        TriggerServerEvent("RNG:setPersonalGangBlipColour", l)
        b = {}
        gangLogs = {}
        e = 1
        f = 1
...
RegisterCommand("gangs", function(a,b,c)
    TriggerServerEvent("RNG:GetGangData")
end)

RegisterNetEvent("RNG:disbandedGang")
AddEventHandler("RNG:disbandedGang",function()
    a = "none"
    PlayerIsInGang = false
    tRNG.setFriendlyFire(false)
    RNGGangMembers = {}
...
RegisterNetEvent("RNG:ForceRefreshData")
AddEventHandler("RNG:ForceRefreshData",function()
    TriggerServerEvent("RNG:GetGangData")
end)

RegisterCommand("nonce", function(a,b,c)
    TriggerEvent("RNG:ForceRefreshData")
end)

RegisterNetEvent("RNG:InviteReceived")
AddEventHandler("RNG:InviteReceived",function(v, w)
    RNGGangInvites[RNGGangInviteIndex] = w
    RNGGangInviteIndex = RNGGangInviteIndex + 1
    tRNG.notify(v)
end)

RegisterNetEvent("RNG:gangNameNotTaken")
AddEventHandler("RNG:gangNameNotTaken",function()
    a = "main"
    PlayerIsInGang = true
    tRNG.setFriendlyFire(true)
end)
...
function func_drawGangUI()
    if a == "none" then
        ...
        DrawAdvancedText(0.558, 0.303, 0.005, 0.0028, 0.539, "RNG Gangs", 255, 255, 255, 255, 7, 0)
        ...
        for x, y in pairs(RNGGangInvites) do
        ...
        if CursorInArea(0.35, 0.415, 0.415, 0.46) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if createGangName ~= nil and createGangName ~= "null" and createGangName ~= "" then
                    TriggerServerEvent("RNG:CreateGang", createGangName)
                else
                    tRNG.notify("~r~No gang name entered!")
                end
            end
        else
            CreateGangSelectionRed = 0
        ...
        if CursorInArea(0.435, 0.51, 0.415, 0.46) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if selectedGangInvite ~= nil then
                    selectedGangInvite = RNGGangInvites[selectedGangInvite]
                    TriggerServerEvent("RNG:addUserToGang", selectedGangInvite)
                    RNGGangInvites = {}
                    a = "main"
                    RNGGangInviteIndex = 0
                    PlayerIsInGang = true
                    ...
                    tRNG.notify("~r~No gang invite selected")
                end
            end
        else
            JoinGangSelectionRed = 0
    ...
    if a == "funds" then
        ...
        DrawAdvancedText(0.591, 0.378, 0.005, 0.0028, 0.48, "RNG Gang - Funds", 255, 255, 255, 255, 7, 0)
        ...
        if CursorInArea(0.3083, 0.3718, 0.5490, 0.5999) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if amount ~= nil then
                    TriggerServerEvent("RNG:depositGangBalance", gangID, amount)
                else
                    tRNG.notify("~r~No amount entered!")
                end
            end
        else
        ...
        if CursorInArea(0.4083, 0.4718, 0.5490, 0.5999) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                TriggerServerEvent("RNG:depositAllGangBalance", gangID)
            end
        else
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if amount ~= nil then
                    if gangPermission >= 3 then
                        TriggerServerEvent("RNG:withdrawGangBalance", amount)
                    else
                        tRNG.notify("~r~You don't have a high enough rank to withdraw")
                    end
                else
                    tRNG.notify("~r~No amount entered!")
                end
        ...
        if CursorInArea(0.6088, 0.6739, 0.5481, 0.6018) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if gangPermission >= 3 then
                    TriggerServerEvent("RNG:withdrawAllGangBalance")
                else
                    tRNG.notify("~r~You don't have a high enough rank to withdraw")
                end
            end
    ...
    if a == "members" then
        ...
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "RNG Gang - Members", 255, 255, 255, 255, 7, 0)
        ...
        for x, z in pairs(b[c]) do
            ...
            local A = h.pinnedPlayers[id] and isAdvanced and "📌" or "⭕"
            DrawAdvancedText(0.700, 0.361 + 0.0287 * x, 0.005, 0.0028, 0.4, A, 255, 255, 255, 255, 6, 0)
            if CursorInArea(0.3005, 0.5955, 0.3731 + 0.0287 * (x - 1), 0.4018 + 0.0287 * (x - 1)) and selectedMember ~= id then
            ...
            if CursorInArea(0.597, 0.610, 0.3731 + 0.0287 * (x - 1), 0.4018 + 0.0287 * (x - 1)) then
                if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                    ...
                    if isAdvanced then
                        ...
                        SetResourceKvp("rng_gang_pinned", json.encode(h.pinnedPlayers))
                    else
                        tRNG.notify("~r~This feature is only available for players with advanced gang license.")
                    end
                end
            end
                ...
                if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                    ...
                    if selectedMember ~= nil and PlayerIsInGang and gangID ~= nil then
                        if gangPermission >= 4 then
                            TriggerServerEvent("RNG:PromoteUser", gangID, tonumber(selectedMember))
                        else
                            tRNG.notify("~r~You don't have permission to promote!")
                        end
                    else
                        tRNG.notify("~r~No gang member selected")
                    end
                end
            else
            ...
            if CursorInArea(0.6182, 0.6822, 0.435, 0.491) then
                ...
                if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                    ...
                    if selectedMember ~= nil and PlayerIsInGang and gangID ~= nil then
                        if gangPermission >= 4 then
                            if selectedMember == tRNG.getUserId() then
                                tRNG.notify("~r~You can't demote yourself.")
                            else
                                TriggerServerEvent("RNG:DemoteUser", gangID, selectedMember)
                            end
                        else
                            tRNG.notify("~r~You don't have permission to demote!")
                        end
                    else
                        tRNG.notify("~r~No gang member selected")
                    end
                    ...
                    if selectedMember ~= nil then
                        if gangPermission >= 3 then
                            if YesNoConfirm() then
                                TriggerServerEvent("RNG:KickUser", gangID, selectedMember)
                            end
                        else
                            tRNG.notify("~r~You don't have permission to kick!")
                        end
                    else
                        tRNG.notify("~r~No gang member selected")
                    end
                ...
                if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                    ...
                    if B ~= nil then
                        if gangPermission >= 2 then
                            TriggerServerEvent("RNG:InviteUser", gangID, B)
                        else
                            tRNG.notify("~r~You don't have permission to invite players")
                        end
                    else
                        tRNG.notify("No player name entered")
                    end
                end
            else
            ...
            if CursorInArea(0.3735, 0.4185, 0.6768, 0.7074) then
                ...
                if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                    if c <= 1 then
                        tRNG.notify("~r~Lowest page reached")
                    else
                        c = c - 1
                    end
                end
            ...
            if CursorInArea(0.4635, 0.5085, 0.6712, 0.7064) then
                ...
                if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                    if c >= e then
                        tRNG.notify("~r~Max page reached")
                    else
                        c = c + 1
                    end
                end
    ...
    if a == "logs" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, h.theme.r, h.theme.g, h.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "RNG Gang - Logs", 255, 255, 255, 255, 7, 0)
        ...
        if CursorInArea(0.33, 0.395, 0.667593, 0.699074) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if isAdvanced then
                    TriggerServerEvent("RNG:SetGangWebhook", gangID)
                end
            end
        else
    ...
    if a == "settings" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, h.theme.r, h.theme.g, h.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "RNG Gang - Settings", 255, 255, 255, 255, 7, 0)
        ...
        if CursorInArea(0.3187, 0.3937, 0.3712, 0.4462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if isAdvanced then
                    h.blips = not h.blips
                    SetResourceKvp("rng_gang_blips", tostring(h.blips))
                else
                    tRNG.notify("~r~This feature is only available for players with advanced gang license.")
                end
            end
        ...
        if CursorInArea(0.4197, 0.4932, 0.3712, 0.4462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if isAdvanced then 
                    h.pings = not h.pings
                    SetResourceKvp("rng_gang_pings", tostring(h.pings))
                else
                    tRNG.notify("~r~This feature is only available for players with advanced gang license.")
                end
            end
        ...
        if CursorInArea(0.3187, 0.3937, 0.4712, 0.5462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if isAdvanced then 
                    h.names = not h.names
                    SetResourceKvp("rng_gang_turf_alerts", tostring(h.names))
                else
                    tRNG.notify("~r~This feature is only available for players with advanced gang license.")
                end
            end
        ...
        if CursorInArea(0.4197, 0.4932, 0.4712, 0.5462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if newGangName ~= nil and newGangName ~= "null" and newGangName ~= "" and gangID ~= nil then
                    TriggerServerEvent("RNG:RenameGang", gangID, newGangName)
                else
                    tRNG.notify("~r~No gang name entered!")
                end
            end
        ...
        if CursorInArea(0.3187, 0.3937, 0.5712, 0.6462) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if YesNoConfirm() then
                    TriggerServerEvent("RNG:LeaveGang", gangID)
                    setCursor(0)
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                if gangPermission >= 4 then
                    if YesNoConfirm() == true and gangID ~= nil then
                        TriggerServerEvent("RNG:DeleteGang", gangID)
                    end
                else
                    tRNG.notify("~r~You don't have permission to disband!")
                end
            end
        else
            DrawRect(0.457, 0.61, 0.075, 0.076, 0, 0, 0, 150)
        end
        DrawAdvancedText(0.451,0.693,0.005,0.0028,0.4,h.healthui and "Disable Health UI" or "Enable Health UI",255,255,255,255,4,0)
        if CursorInArea(0.31, 0.39, 0.6712, 0.7064) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                SetResourceKvp("rng_gang_healthui", tostring(h.healthui))
            end
        else
        ...
        if CursorInArea(0.41, 0.49, 0.6712, 0.7064) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if gangPermission >= 4 then
                    TriggerServerEvent("RNG:setGangFit", gangID)
                else
                    tRNG.notify("~r~You don't have permission to set gang fit!")
                end
            end
        else
        ...
        if CursorInArea(0.595, 0.645, 0.6155, 0.6405) then
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                ...
                SetResourceKvp("rng_gang_colour", l)
                TriggerServerEvent("RNG:setPersonalGangBlipColour", l)
            end
        end
        DrawAdvancedText(0.775, 0.693, 0.005, 0.0028, 0.4, "Back", 255, 255, 255, 255, 4, 0)
    ...
    if a == "turfs" then
        ...
        DrawRect(0.501, 0.308, 0.421, 0.047, h.theme.r, h.theme.g, h.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "RNG Gang - Turfs", 255, 255, 255, 255, 7, 0)
    ...
    if a == "security" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, h.theme.r, h.theme.g, h.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "RNG gang - security", 255, 255, 255, 255, 7, 0)
        ...
        if CursorInArea(0.31, 0.65, 0.56, 0.61) then
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if gangPermission >= 4 then
                    TriggerServerEvent("RNG:LockGangFunds", gangID)
                else
    ...
    if a == "theme" then
        DrawRect(0.501, 0.525, 0.421, 0.387, 0, 0, 0, 150)
        DrawRect(0.501, 0.308, 0.421, 0.047, h.theme.r, h.theme.g, h.theme.b, 248)
        DrawAdvancedText(0.591, 0.312, 0.005, 0.0028, 0.48, "RNG Gang - Theme", 255, 255, 255, 255, 7, 0)
        ...
        DrawAdvancedText(0.490,0.405,0.005,0.0028,0.48,"(" .. h.theme.r .. "," .. h.theme.g .. "," .. h.theme.b .. ")",255,255,255,255,6,0)
        ...
        if CursorInArea(0.5187, 0.5828, 0.4138, 0.4694) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                tRNG.CopyToClipboard(h.theme.r .. "," .. h.theme.g .. "," .. h.theme.b)
                tRNG.notify("~g~Theme copied to clipboard.")
            end
        else
        ...
        if CursorInArea(0.5187, 0.5828, 0.5407, 0.5962) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                tRNG.gangMenuTheme(math.random(1, 255), math.random(1, 255), math.random(1, 255))
            end
        else
        ...
        if CursorInArea(0.6182, 0.6822, 0.5407, 0.5962) then
            ...
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                tRNG.clientPrompt("Enter rgb value eg 255,100,150:","",function(L)
                    if L ~= "" then
                        local M = stringsplit(L, ",")
                        if M[1] ~= nil and M[2] ~= nil and M[3] ~= nil then
                            tRNG.gangMenuTheme(tonumber(M[1]), tonumber(M[2]), tonumber(M[3]))
                        else
                            tRNG.notify("~r~Invalid value")
                        end
                    else
                        tRNG.notify("~r~Invalid value")
                    end
    ...
    if a == "main" then
        ...
        DrawAdvancedText(0.591, 0.399, 0.005, 0.0028, 0.51, "RNG Gangs", 255, 255, 255, 255, 7, 0)
        ...
        if CursorInArea(0.558, 0.6216, 0.5722, 0.6259) then
            DrawRect(0.59, 0.6, 0.065, 0.056, red, green, blue, 150)
            if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                if isAdvanced then
                    a = "theme"
...
tRNG.createThreadOnTick(b5)

Citizen.CreateThread(function()
    local N = json.decode(GetResourceKvpString("rng_gang_pinned") or "{}") or {}
    h = {
        blips = GetResourceKvpString("rng_gang_blips") == "true",
        pings = GetResourceKvpString("rng_gang_pings") == "true",
        names = GetResourceKvpString("rng_gang_turf_alerts") == "true",
        healthui = GetResourceKvpString("rng_gang_healthui") == "true",
        pinnedPlayers = {}
    }
    for O in pairs(N) do
        h.pinnedPlayers[tonumber(O)] = true
    end
    h.theme = json.decode(GetResourceKvpString("rng_gang_theme")) or {r = 18, g = 82, b = 228}
    while true do
...
RegisterNetEvent("RNG:Notify",function(j)
    tRNG.notify(j)
end)

RegisterNetEvent("RNG:setGangMemberColour",function(P, K)
    for h, k in pairs(RNGGangMembers) do
        if k[2] == P then
            RNGGangMembers[h].colour = K
        end
    end
end)

function tRNG.gangMenuTheme(Q, R, S)
    if Q ~= nil and R ~= nil and S ~= nil then
        h.theme = {r = Q, g = R, b = S}
        SetResourceKvp("rng_gang_theme", json.encode(h.theme))
    else
        tRNG.notify("~r~Invalid value")
    end
end

function tRNG.hasGangNamesEnabled()
    local M = m()
    return M and isAdvanced and h.names
end

function tRNG.isPlayerInSelectedGang(O)
    local M = m()
    if M then
        local P = tRNG.clientGetUserIdFromSource(O)
        if P and tRNG.getJobType(P) == "" then
            for T, U in pairs(RNGGangMembers) do
                if P == U[2] then
...
function tRNG.hasGangBlipsEnabled()
    if h ~= nil and isAdvanced and h.blips then
        return true
    end
    return false
end
...
RegisterCommand("drawmarker",function()
    if h.pings and isAdvanced and not tRNG.isEmergencyService() and not globalHideUi and not tRNG.inEvent() then
        local X = GetGameplayCamCoord()
        local Y = tRNG.rotationToDirection(GetGameplayCamRot(2))
        ...
        if _ ~= vector3(0.0, 0.0, 0.0) then
            TriggerServerEvent("RNG:sendGangMarker", gangID, _)
        end
    end
end)

local function a0(a1)
    if tRNG.getGangPingMarkerIndex() == 2 and not tRNG.isEmergencyService() and not tRNG.inEvent() then
        local a2, a3 = GetGroundZFor_3dCoord(a1.x, a1.y, a1.z, a1.z, false)
        local a4 = math.abs(a3 - a1.z)
        local a5 = (a4 > 10.0 and a1.z or a3) - 1.0
        return CreateCheckpoint(47,a1.x,a1.y,a5,a1.x,a1.y,a1.z + 200.0,1.0,h.theme.r,h.theme.g,h.theme.b,150,0)
    else
        return nil
    end
end
local function a6(a1)
    if tRNG.displayPingsOnMinimap() then
...
RegisterNetEvent("RNG:drawGangMarker")
AddEventHandler("RNG:drawGangMarker",function(a7, a8)
    if h.pings and isAdvanced and not tRNG.isEmergencyService() and not globalHideUi and not tRNG.inEvent() then
        ...
        if a9 < 1000.0 then
            ...
            if a8 then
                ...
                if tRNG.getGangPingSound() > 1 then
                    SendNUIMessage({transactionType = "gangping" .. tRNG.getGangPingSound() - 1,volume = tRNG.getGangPingVolume() * 0.1})
                end
            else
                W[a7] = nil
                V[a7] = nil
...
Citizen.CreateThread(function()
    while true do
        if h ~= nil and isAdvanced and h.pings and PlayerIsInGang then
...
Citizen.CreateThread(function()
    while true do
        if h ~= nil and isAdvanced and h.pings and PlayerIsInGang then
            for T, U in pairs(V) do
                if V[T] ~= nil then
                    if tRNG.getGangPingMarkerIndex() == 3 then
                        tRNG.DrawSprite3d(
                            {
                    ...
                    tRNG.DrawText3D(U.coords, U.creator .. "\n" .. tostring(math.floor(U.dist)) .. "m", 0.2)
                end
            end
        end
        Citizen.Wait(0)
...
local ab = {}
RegisterNetEvent("RNG:sendGangHPStats")
AddEventHandler("RNG:sendGangHPStats",function(ac)
    ab = ac
end)
...
local function al()
    local am = 0
    local M = m()
    if h ~= nil and isAdvanced and h.healthui and PlayerIsInGang then
        if M and not tRNG.isEmergencyService() and not globalHideUi and not tRNG.inEvent() then
            local an = 0
            local ao = tRNG.getShowHealthPercentageFlag()
            if RNGGangMembers ~= nil then
                for ap, aq in pairs(RNGGangMembers) do
                    name, id, rank, lastseen, playtime = table.unpack(aq)
                    if lastseen == "~g~Online" and tRNG.getJobType(id) == "" and h.pinnedPlayers[id] then
                        local ar = true
                        local as = nil
                        local at = nil
                        local ac = ab[id]
                        ...
                        local au = tRNG.getTempFromPerm(id)
                        if au then
                        ...
                        if as and at then
                            ...
                            if ao then
                                ...
                                tRNG.DrawText(ad + 0.125 / 2.0 - 0.0025,ax + 0.03,tostring(aA) .. "%",0.15,nil,nil,af)
                            end
                            an = an + 1
                        end
                        if ar then
    ...
    if i and i == M then
        if am <= 0 then
            TriggerServerEvent("RNG:getGangHealthTable", nil)
            i = nil
        end
    else
        if am > 0 then
            TriggerServerEvent("RNG:getGangHealthTable", gangID)
            i = M
        end
    end
end

tRNG.createThreadOnTick(al)

function tRNG.setGangUIXPos(aB)
    local aC = tonumber(string.match(aB, "%d+")) / 100
    ad = 0.008 * aC
end

function tRNG.setGangUIYPos(aB)
    local aC = tonumber(string.match(aB, "%d+")) / 100
    ae = 0.35 * aC
end