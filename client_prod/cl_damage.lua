...
function GameEventTriggered(i, j)
    if i == "CEventNetworkEntityDamage" then
        ...
        if m and l == PlayerPedId() and IsEntityAPed(k) and k ~= PlayerPedId() then
            TriggerEvent("RNG:onPlayerKilledPed", k)
        end
        if k == PlayerPedId() then
            local t = tRNG.getPedServerId(l)
            if n == 0 then
                ...
                function()
                    ...
                    if DecorExistOn(PlayerPedId(), "lasthp") then
                        ...
                        if u < v then
                            if c[r] then
                                if t then
                                    TriggerServerEvent("RNG:syncEntityDamage", u, v, t, true, m, n)
                                end
                            elseif d[r] then
                                if t then
                                    TriggerServerEvent("RNG:syncEntityDamage", u, v, t, false, m, n)
                                end
                            end
                        end
                        if m then
        ...
        if
            tRNG.getDmgIndcator() and (tRNG.isPlusClub() or tRNG.isPlatClub()) and IsEntityAPed(k) and k ~= PlayerPedId() and
                l == PlayerPedId()
         then
            ...
            if x then
                ...
                Citizen.CreateThread(
                    function()
                        while true do
                            if A >= 0 then
                                tRNG.DrawText3D(vector3(B.x, B.y, B.z + 0.5), w, 0.5, 4, nil, z[y], true, true)
                                A = A - 10
                            end
                            Wait(0)
                        end
...
RegisterNetEvent("RNG:newKillFeed",function(d, e, f, g, h, i, j, headshotKill)
...
AddEventHandler(
    ...
    function(C, D, n, E)
        ...
        if D == F and C ~= F and IsEntityAPed(C) and GetEntityHealth(C) > 105 and not IsEntityDead(C) then
            Citizen.Wait(0)
            TriggerEvent("RNG:onPlayerDamagePed", C)
        end
    end
)
CreateThread(
...
local A = 0
RegisterNetEvent(
    "RNG:onEntityHealthChange",
    function(G, u, v, H)
        if f and (tRNG.isPlusClub() or tRNG.isPlatClub()) then
