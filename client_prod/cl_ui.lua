...
local l = 10.0
local m = {0, 0, 0, 255}
function tRNG.setMaxUnderWaterUITimenewTime(n)
    l = n
end
local function o(p, q, r, s, t, u, v, w)
    DrawRect(p + r / 2, q + s / 2, r, s, t, u, v, w)
end
function tRNG.getCachedMinimapAnchor()
    return k
end
function tRNG.getCachedResolution()
        ...
        while true do
            if not globalHideUi then
                if not globalHideEmergencyCallUI and not glifeheathui and not fortnitehealthui then
                    ...
                    local E = tRNG.getShowHealthPercentageFlag()
                    if C > 1.0 then
                        C = 1.0
                    end
...
function tRNG.setHealthUIType(a)
    F = true
    if a == "RNG" then
        glifeheathui = false
        fortnitehealthui = false
