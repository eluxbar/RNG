RMenu.Add(
    "cardev",
    "mainmenu",
    RageUI.CreateMenu("", "", tRNG.getRageUIMenuWidth(), tRNG.getRageUIMenuHeight(), "rng_adminui", "rng_adminui")
)
    ...
    function()
        if RageUI.Visible(RMenu:Get("cardev", "mainmenu")) then
            RageUI.DrawContent(
                ...
                function()
                    if b and tRNG.getPlayerBucket() == 333 then
                        RageUI.ButtonWithStyle(
                            "Check Tickets",
                            "~o~Check all pending car dev tickets",
                            {RightLabel = "→→→"},
                            true,
                            function(Hovered, Active, Selected)
                                if Selected then
                                    TriggerServerEvent("RNG:RequestCarTickets")
                                end
                            end,
                            RMenu:Get("cardev", "tickets")
                        )
                        RageUI.ButtonWithStyle(
                            ...
                            function(h, i, j)
                                if j then
                                    tRNG.clientPrompt(
                                        ...
                                        "",
                                        function(k)
                                            if k ~= "" and tRNG.getPlayerBucket() == 333 then
                                                local l = tRNG.loadModel(k)
                                                TriggerServerEvent("RNG:logVehicleSpawn", k, "/cardev")
                                                local m = tRNG.getPlayerCoords()
                                                local n =
                                                    tRNG.spawnVehicle(
                                                    l,
                                                    m.x,
                                                ...
                                                SetPedIntoVehicle(tRNG.getPlayerPed(), n, -1)
                                    ...
                                    tRNG.clientPrompt(
                                        ...
                                        function(k)
                                            if k ~= "" and tRNG.getPlayerBucket() == 333 then
                                                ...
                                                SetVehRadioStation(n, "OFF")
                                                if k == nil then 
                                                    tRNG.notify("The vehicle model could not be loaded. Please check the spawncode and try again.")
                                                end
                                            end
                                        end
                                    )
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(h, i, j)
                                if j then
                                    local o = GetVehiclePedIsIn(tRNG.getPlayerPed(), false)
                        ...
                        RageUI.Checkbox(
                            ...
                            function()
                                b = true
                                TriggerServerEvent("RNG:setCarDevMode", b)
                            end,
                            function()
                        ...
                        RageUI.Checkbox(
                            ...
                            "~g~Enter the Car Dev Universe to see more menu options.",
                            b,
                            {},
                            function()
                            end,
        ...
        -- if RageUI.Visible(RMenu:Get("cardev", "tickets")) then
        --     RageUI.DrawContent(
        --         {header = true, glare = false, instructionalButton = false},
        --         function()
        --             RageUI.ButtonWithStyle(
        ...
        --                 function()
        --                 end,
        --                 RMenu:Get("cardev", "actualtickets")
        --             )
        --         end
        --     )
        -- end
        if RageUI.Visible(RMenu:Get("cardev", "tickets")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = false},
                function()
                    if #cardevstats > 0 then
                        for _, tickets in ipairs(cardevstats) do
                            RageUI.ButtonWithStyle(
                                tickets.spawncode, 
                                "Report ID: "..tickets.reportid .. " \nDiscord ID: " .. tickets.reporter .. " \nNotes: " .. tickets.issue, 
                                {RightLabel = "→→→"}, 
                                true, 
                                function()
                                    -- This function is empty, add functionality here if needed
                                end,
                                RMenu:Get("cardev", "completetickets")
                            )
                        end
                    else
                        RageUI.Separator("~r~There Are 0 reports")
                    end
                end
            )
        end
        
        if RageUI.Visible(RMenu:Get("cardev", "completetickets")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = false},
                function()
                    RageUI.ButtonWithStyle(
                        ...
                        function(Hovered, Active, Selected) 
                            if Selected then 
                                TriggerServerEvent("RNG:completeticket")
                            end
                        end
                    )
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(h, i, j)
                            if j then
                                local spawncode = cardevstats[1] and cardevstats[1].spawncode
                                if spawncode ~= "" and tRNG.getPlayerBucket() == 333 then
                                    local l = tRNG.loadModel(spawncode)
                                    TriggerServerEvent("RNG:logVehicleSpawn", spawncode, "/cardev")
                                    local m = tRNG.getPlayerCoords()
                                    local n =
                                        tRNG.spawnVehicle(
                                        l,
            ...
            RageUI.DrawContent(
                ...
                function()
                    for w, x in pairs(f) do
                        if GetNumVehicleMods(tRNG.getPlayerVehicle(), w) > 0 then
                            RageUI.ButtonWithStyle(
                                x,
                                nil,
                                {RightLabel = "→→→"},
        ...
        if RageUI.Visible(RMenu:Get("cardev", "vehiclemodindexes")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = false},
                function()
                    if GetNumVehicleMods(tRNG.getPlayerVehicle(), selectedModType) == 0 then
                        RageUI.Text({message = "No available mod indexes for this mod type for this vehicle."})
                    else
                        for w = 0, GetNumVehicleMods(tRNG.getPlayerVehicle(), selectedModType) do
                            RageUI.ButtonWithStyle(
                                "Mod " .. w,
                                nil,
                                {RightLabel = "→→→"},
                                ...
                                function(h, i, j)
                                    if j then
                                        SetVehicleModKit(tRNG.getPlayerVehicle(), 0)
                                        SetVehicleMod(tRNG.getPlayerVehicle(), selectedModType, w)
                                    end
                                end,
                                RMenu:Get("cardev", "vehiclemodindexes")
                            )
                                    ...
                                    RageUI.Button(
                                        ...
                                        function(h, i, j)
                                            if j then
                                                ...
                                                TriggerEvent("RNG:staffFixVehicle")
                                            end
                                        end
                                    )
                                end
                ...
                function()
                    for z, A in pairs(g) do
                        RageUI.Button(
                            ...
                            function(h, i, j)
                                if j then
                                    SetVehicleColours(tRNG.getPlayerVehicle(), A.colorindex, A.colorindex)
                                end
                            end
                        )
                    end
...
RegisterCommand(
    "cardev",
    function(C)
        if a and not tRNG.isPurge() then
            B()
        end
    end
)
...
local function ae()
    if not b or tRNG.getPlayerBucket() ~= 333 then
        if J then
            ac(false)
        end
        return
    ...
    L()
    local af = tRNG.getFontId("Akrobat-ExtraBold")
    local ag = J and 0.345 or 0.505
    if not TakingCarSS then
...
function TakeVehScreenshot()
    ...
    Wait(13000)
    TriggerServerEvent('RNG:takeCarScreenshot')
    Wait(5000)
...
RegisterNetEvent("RNG:setCarTickets")
AddEventHandler("RNG:setCarTickets", function(tickets)
    cardevstats = tickets
end)