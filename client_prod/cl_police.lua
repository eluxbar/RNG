...
local e = false
local f = false
function tRNG.isKnockedOut()
    return a
end
function tRNG.putInNearestVehicleAsPassenger(g)
    local h = tRNG.getClosestVehicle(g)
    if IsEntityAVehicle(h) then
        for i = 1, math.max(GetVehicleMaxNumberOfPassengers(h), 3) do
            if IsVehicleSeatFree(h, i) then
                SetPedIntoVehicle(tRNG.getPlayerPed(), h, i)
                return true
            end
        end
    end
...
function tRNG.putInNetVehicleAsPassenger(j)
    local h = tRNG.getObjectId(j)
    if IsEntityAVehicle(h) then
...
function tRNG.putInVehiclePositionAsPassenger(k, l, m)
    local h = tRNG.getVehicleAtPosition(k, l, m)
    if IsEntityAVehicle(h) then
        for i = 1, GetVehicleMaxNumberOfPassengers(h) do
            if IsVehicleSeatFree(h, i) then
                SetPedIntoVehicle(tRNG.getPlayerPed(), h, i)
...
RegisterNetEvent(
    "RNG:knockOut",
    function()
        if not a and not tRNG.isPurge() then
    ...
    function()
        while true do
            ...
            if IsControlPressed(0, 323) or IsControlPressed(0, 27) and not IsUsingKeyboard(2) then
                if
                    not globalSurrenderring and not tRNG.isInComa() and not tRNG.isHandcuffed() and
                        (tRNG.canAnim() or tRNG.isTazedByRevive())
                 then
                    DisablePlayerFiring(tRNG.getPlayerPed(), true)
                    DisableControlAction(0, 22, true)
                    DisableControlAction(0, 25, true)
                    DisableControlAction(0, 154, true)
                    if not IsEntityDead(tRNG.getPlayerPed()) then
                        if not c and not GetIsTaskActive(tRNG.getPlayerPed(), 298) then
                            c = true
                            tRNG.loadAnimDict("missminuteman_1ig_2")
                            TaskPlayAnim(
                                tRNG.getPlayerPed(),
                                "missminuteman_1ig_2",
                                "handsup_enter",
            ...
            if a then
                if tRNG.isStaffedOn() then
                    TriggerEvent("RNG:knockOutDisable")
                elseif not tRNG.isInComa() then
                    local p = PlayerPedId()
                    if not IsEntityPlayingAnim(p, b[1], b[2], 3) then
                        tRNG.loadAnimDict(b[1])
                    ...
                    tRNG.notify("~r~You have been knocked out!")
                end
            end
            Wait(0)
        end
...
RMenu.Add(
    "policehandbook",
    "main",
    RageUI.CreateMenu("Police Handbook", "~b~Officer Handbook", tRNG.getRageUIMenuWidth(), tRNG.getRageUIMenuHeight())
)
RageUI.CreateWhile(
    1.0,
    true,
            ...
            for P, Q in pairs(O) do
                if GetPlayerPed(Q) ~= B then
                    ...
                    if S < 3.0 then
                        ...
                        TriggerServerEvent("RNG:breathalyserRequest", T)
                        break
                    end
                end
            end
...
RegisterNetEvent(
    "RNG:flashWarrantCard",
    function()
        local B = PlayerPedId()
        local U = tRNG.loadModel("prop_fib_badge")
...
RegisterNetEvent(
    "RNG:startSearchingSuspect",
    function()
        tRNG.setCanAnim(false)
        tRNG.loadAnimDict("custom@police")
        ...
        while GetGameTimer() - W < 10000 do
            if IsDisabledControlJustPressed(0, 73) then
                TriggerServerEvent("RNG:cancelPlayerSearch")
                return
            end
            Citizen.Wait(0)
        end
...
RegisterNetEvent(
    ...
    function(Y)
        ...
        X = true
        tRNG.setCanAnim(false)
        tRNG.loadAnimDict("custom@suspect")
...
RegisterNetEvent("RNG:receivePoliceCallsign")
AddEventHandler("RNG:receivePoliceCallsign", function(a8, a9, aa)
    a0 = a9
    a1 = a8
    a2 = aa
    a3 = true
...
RegisterNetEvent("RNG:receiveHmpCallsign")
AddEventHandler("RNG:receiveHmpCallsign", function(a8, a9, aa)
    a4 = a9
    a5 = a8
    a6 = aa
    a7 = true
...
function tRNG.getPoliceCallsign()
    return a1
end

function tRNG.getPoliceRank()
    return a0
end

function tRNG.getPoliceName()
    return a2
end

function tRNG.hasPoliceCallsign()
    return a3
end

function tRNG.getHmpCallsign()
    return a5
end

function tRNG.getHmpRank()
    return a4
end

function tRNG.getHmpName()
    return a6
end

function tRNG.hasHmpCallsign()
    return a7
end
...
tRNG.createThreadOnTick(func_drawCallsign)
local ab = 0
local function ac()
    local p = tRNG.getPlayerPed()
...
tRNG.createThreadOnTick(ac)
function tRNG.hasRecentlyShotGun()
...
RMenu.Add(
    "trainingWorlds",
    "mainmenu",
    RageUI.CreateMenu("Training Worlds", "Main Menu", tRNG.getRageUIMenuWidth(), tRNG.getRageUIMenuHeight())
)
local ah = {}
local ai = false
RegisterNetEvent(
    "RNG:trainingWorldOpen",
    function(aj)
        ai = aj
        ...
        RageUI.IsVisible(
            ...
            function()
                ...
                for u, al in pairs(ah) do
                    ...
                    local an = al.bucket == tRNG.getPlayerBucket()
                        ...
                        function(F, G, H)
                            if G and ai then
                                ...
                                if IsControlJustPressed(0, 214) then
                                    TriggerServerEvent("RNG:trainingWorldRemove", u)
                                end
                            end
                            if H then
                                TriggerServerEvent("RNG:trainingWorldJoin", u)
                            end
                        end
                    )
                    if an then
                ...
                if ak then
                    RageUI.ButtonWithStyle(
                        ...
                        function(F, G, H)
                            if H then
                                TriggerServerEvent("RNG:trainingWorldLeave")
                            end
                        end
                    )
                end
                if ai then
                    RageUI.ButtonWithStyle(
                        ...
                        function(F, G, H)
                            if H then
                                TriggerServerEvent("RNG:trainingWorldCreate")
                            end
                        end
                    )
                end
...
RegisterNetEvent(
    "RNG:trainingWorldSend",
    function(u, ap)
        ah[u] = ap
    end
)
RegisterNetEvent(
    "RNG:trainingWorldSendAll",
    function(ap)
        ah = ap
    end
)
RegisterNetEvent(
    "RNG:trainingWorldRemove",
    function(u)
        ah[u] = nil
    end
)
function tRNG.canBeCuffed()
    return tRNG.isPlayerSurrenderedNoProgressBar(), tRNG.isHandcuffed() or tRNG.isPedBeingTackled() or tRNG.isTazed()
end
