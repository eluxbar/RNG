...
local function u()
    ...
    if v ~= 1 then
        ...
        local z = tRNG.rotationToDirection(x)
...
local function G(H)
    ...
    if noclipActive and tRNG.getStaffLevel() == 0 then
        return
    end
    inventoryType = nil
    D = true
    ...
    if tRNG.globalOnPoliceDuty() then
        tRNG.loadAnimDict("amb@medic@standing@kneel@base")
        TaskPlayAnimAdvanced(t,"amb@medic@standing@kneel@base","base",F.x,F.y,F.z,J.x,J.y,J.z,8.0,-8.0,-1,524288,512,0,false,false)
        RemoveAnimDict("amb@medic@standing@kneel@base")
        ForcePedAiAndAnimationUpdate(t, false, false)
        Citizen.Wait(200)
        while D do
            if not IsEntityPlayingAnim(PlayerPedId(), "amb@medic@standing@kneel@base", "base", 3) or IsControlJustPressed(0, 137) then
                D = false
        ...
        tRNG.loadAnimDict("amb@medic@standing@kneel@base")
        TaskPlayAnimAdvanced(t,"amb@medic@standing@kneel@base","base",F.x,F.y,F.z,J.x,J.y,J.z,8.0,-8.0,-1,1,0,false,false)
        RemoveAnimDict("amb@medic@standing@kneel@base")
        ForcePedAiAndAnimationUpdate(t, false, false)
    end
    E(F)
                        ...
                        if not g then
                            if GetHashKey("xs_prop_arena_bag_01") == L then
                                ...
                                if B() then
                                    ...
                                    G(
                                        function()
                                            TriggerServerEvent("RNG:LootBag", N)
                                        end
                                    )
                                    Wait(1000)
                                end
                            elseif GetHashKey("prop_poly_bag_money") == L then
                                ...
                                if B() then
                                    ...
                                    local P = ObjToNet(O)
                                    TriggerEvent("RNG:startCombatTimer", false)
                                    TriggerServerEvent("RNG:Moneydrop", P)
                                    Wait(1000)
                                end
                            ...
                            elseif GetHashKey("xs_prop_arena_crate_01a") == L then
                                ...
                                if B() then
                                    ...
                                    G(
                                        function()
                                            TriggerServerEvent("RNG:openCrate", Q)
                                            inventoryType = "Crate"
                                        end
                                    )
                                    Wait(1000)
...
local function aa(ab)
    TriggerEvent("RNG:lockNearestVehicle")
end
local ac
local function ad(ab)
    ac = ab
    SetVehicleDoorOpen(ab, 5, true, false)
    TriggerEvent("RNG:clOpenTrunk")
    trunkStatus = true
    SendNUIMessage({closeRadialMenu = true})
    local ae = GetSoundId()
...
RegisterNetEvent("RNG:clCloseTrunk")
AddEventHandler(
    "RNG:clCloseTrunk",
    function()
        if ac then
...
local function ag(ab)
    TriggerEvent("RNG:verifyLockpick", ab)
end
local function ah(ai)
    local aj = GetPedInVehicleSeat(ai, -1)
    if aj ~= 0 then
        return tRNG.notify("~r~Can not repair vehicle with a person in the driver seat.")
    end
    if NetworkGetEntityIsNetworked(ai) then
        local ak = NetworkGetNetworkIdFromEntity(ai)
        if ak ~= 0 then
            TriggerServerEvent("RNG:attemptRepairVehicle", ai, ak)
        end
    end
end
local al = false
...
local function an(ab)
    if f then
        TriggerEvent("RNG:searchClient", ab)
    end
end
local function ao(ab)
    if f then
        local ap = tonumber(DecorGetInt(ab, "rng_owner"))
        if ap > 0 then
            exports.rng:impound(ap, GetEntityModel(ab), VehToNet(ab), ab)
        else
            TriggerEvent("RNG:Notify", "~r~Vehicle is not owned by anyone")
            if GetPedInVehicleSeat(ab, -1) == 0 and GetPedInVehicleSeat(ab, 0) == 0 and NetworkGetEntityIsNetworked(ab) then
                TriggerServerEvent("RNG:impoundDeleteVehicle", NetworkGetNetworkIdFromEntity(ab))
            end
        end
    end
end
local function aq(ab)
    ...
    if as > 0 then
        if GetSelectedPedWeapon(PlayerPedId()) ~= GetHashKey("WEAPON_UNARMED") then
            TriggerServerEvent("RNG:robPlayer", as)
        else
            TriggerEvent("RNG:Notify", "~r~You need a weapon in your hands.")
        end
    end
end
local function at(ab)
    ...
    if aj ~= 0 then
        ...
        if as > 0 then
            TriggerServerEvent("RNG:askId", as)
        end
    end
end
local au = 0
...
local function aw(ab)
    ...
    local as = GetPlayerServerId(ar)
    if as > 0 then
        TriggerServerEvent("RNG:giveCashToPlayer", as)
    end
end
local function ax(ab)
    if not tRNG.canAnim() then
        return
    end
    if GetEntityHealth(ab) <= 102 then
        TriggerEvent("RNG:Notify", "~r~You can not search a player who is dead.")
        return
    end
    local ar = GetPlayerByEntityID(ab)
        ...
        if ay ~= nil then
            if
             ...
             then
                local as = GetPlayerServerId(ar)
                if as > 0 then
                    TriggerServerEvent("RNG:searchPlayer", as)
                end
            else
                TriggerEvent("RNG:Notify", "~r~Player must have their hands up or be on their knees!")
            end
        end
    else
        local as = GetPlayerServerId(ar)
...
local function az(ab)
    ...
    if as > 0 then
        if g then
            TriggerServerEvent("RNG:nhsRevive", as)
        else
            TriggerServerEvent("RNG:attemptCPR", as)
        end
    end
end
local function aA(ab)
...
local function aB(ab)
    if f or h then
        ...
        local as = GetPlayerServerId(ar)
        if as > 0 then
            TriggerServerEvent("RNG:dragPlayer", as)
        end
    end
end
local function aC(ab)
    if f or h then
        ...
        local as = GetPlayerServerId(ar)
        if as > 0 then
            TriggerServerEvent("RNG:putInVehicle", as)
        end
    end
end
local function aD(ab)
    ...
    local as = GetPlayerServerId(ar)
    if as > 0 then
        TriggerServerEvent("RNG:gunshotTest", as)
    end
end
local function aE(ab)
    if f or h then
        ...
        local as = GetPlayerServerId(ar)
        if as > 0 then
            TriggerServerEvent("RNG:seizeWeapons", as)
            TriggerServerEvent("RNG:jailPlayer", as)
        end
    end
end
local function aF(ab)
    if f or h then
        ...
        local as = GetPlayerServerId(ar)
        if as > 0 then
            TriggerServerEvent("RNG:requestTransport", as)
        end
    end
end
local function aG(ab)
...
local function aH(ab)
    if f or h then
        ...
        local as = GetPlayerServerId(ar)
        if as > 0 then
            TriggerServerEvent("RNG:seizeIllegals", as)
        end
    end
end
RegisterNUICallback(
    ...
    function(o)
        ...
        elseif aI == "leaveRadio" then
            TriggerEvent("RNG:clientLeaveRadioChannel")
        elseif aI == "toggleMute" then
            ExecuteCommand("toggleradiomute")
        elseif aI == "radioConfig" then
            TriggerEvent("RNG:openRadioConfig")
        elseif string.match(aI, "radioChannel") then
            local aJ = string.sub(aI, 13, #aI)
            TriggerEvent("RNG:clientLeaveRadioChannel")
            Wait(2000)
            TriggerEvent("RNG:clientJoinRadioChannel", tonumber(aJ))
        end
    end
)
RegisterNetEvent("RNG:globalOnPoliceDuty")
AddEventHandler(
    "RNG:globalOnPoliceDuty",
    function(aK)
        f = aK
    end
)
RegisterNetEvent("RNG:globalOnNHSDuty")
AddEventHandler(
    "RNG:globalOnNHSDuty",
    function(aK)
        g = aK
...
function tRNG.getSelectedEntity()
    return b, c
end
AddEventHandler(
    "RNG:showRadioWheel",
...
RegisterCommand(
    "lootclosestbag",
    function()
        if not playerIsAlive() or tRNG.getPlayerVehicle() ~= 0 or GetRenderingCam() ~= -1 then
            return
        end
        local t = tRNG.getPlayerPed()
        local aO = tRNG.getPlayerCoords()
        local aP = 0
        local aQ = 3.0
        ...
        if aP ~= 0 then
            ...
            if not g then
                ...
                elseif aS == GetHashKey("prop_box_ammo03a") then
                    ...
                    G(
                        function()
                            TriggerServerEvent("RNG:openCrate", aT)
                        end
                    )
...
RegisterNetEvent("RNG:playZipperSound",function(j)
