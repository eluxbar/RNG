...
local function f()
    ...
    g.position = tRNG.getPlayerCoords()
    g.radius = g.radius + 0.0
    g.vehicles.speed = g.vehicles.speed + 0.0
    TriggerServerEvent("RNG:createRPZone", g)
end
...
local function k()
    ...
    if l then
        TriggerServerEvent("RNG:removeRPZone", l)
    end
end
local p = false
RMenu.Add(
...
local function w(g, x)
    ...
    elseif not g.weapons.shotgun and y == GetHashKey("GROUP_SHOTGUN") then
        tRNG.setWeapon(j, "WEAPON_UNARMED")
    elseif not g.weapons.mg and y == GetHashKey("GROUP_MG") then
        tRNG.setWeapon(j, "WEAPON_UNARMED")
...
local function I(g)
    local n, x = GetCurrentPedWeapon(tRNG.getPlayerPed())
    if x ~= 0 then
        w(g, x)
    end
    local A, J = tRNG.getPlayerVehicle()
    if A ~= 0 and J then
        C(g, A)
    end
...
tRNG.createThreadOnTick(K)
function tRNG.GetRPZoneInfo()
    local M = tRNG.getPlayerCoords()
    for n, g in ipairs(d) do
        if #(M - g.position) < g.radius then
            return g
