RMenu.Add(
    "devmenu",
    "main",
    RageUI.CreateMenu("RNG Dev Tools", "Developer Tools", tRNG.getRageUIMenuWidth(), tRNG.getRageUIMenuHeight(),"",""))
RMenu.Add(
    "devmenu",
    "animations",
...
local z = module("rng-weapons", "cfg/weapons")
        ...
        if RageUI.Visible(RMenu:Get("devmenu", "main")) then
            RageUI.DrawContent(
                ...
                function()
                    ...
                    --             tRNG.clientPrompt(
                    --                 "Customization [JSON]",
                    --                 json.encode(tRNG.getCustomization()),
                    --                 function()
                    --                 end
                    --             )
                    --         end
                    ...
                    --                 "",
                    --                 function(D)
                    --                     tRNG.setCustomization(json.decode(D))
                    --                 end
                    --             )
                    --         end
                    --     end
                    ...
                    --                 "",
                    --                 function(G)
                    --                     local H = tRNG.loadModel(tonumber(G))
                    --                     if H then
                    --                         local I =
                    --                             GetEntityCoords(PlayerPedId()) +
                    --                             GetEntityForwardVector(PlayerPedId()) * 2.0
                    ...
                    --     function(A, B, C)
                    --         if C then
                    --             TriggerEvent("RNG:toggleTrafficMenu")
                    --         end
                    --     end
                    -- )
                    RageUI.Button(
                        "Create Women Test Ped",
                        nil,
                        {},
                        true,
                        function(A, B, C)
                            if C then
                                local J = tRNG.loadModel("a_f_m_beach_01")
                    ...
                    RageUI.Button(
                        "Create Male Test Ped",
                        nil,
                        {},
                        true,
                        function(A, B, C)
                            if C then
                                local J = tRNG.loadModel("s_m_m_pilot_02")
                                local K = tRNG.getPlayerCoords()
                                CreatePed(1, J, K.x, K.y, K.z, 0.0, true, true)
                                SetModelAsNoLongerNeeded(J)
                            end
                    ...
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                ...
                                notify("~g~All Peds, Vehicles, Props Have Been Cleared")
                            end
                        end
                    )
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                TriggerServerEvent('RNG:CleanVeh')
                                notify("~g~All Vehicles Cleared")
                            end
                        end
                    )
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                TriggerServerEvent('RNG:CleanObj')
                                notify("~g~All Unwanted Objects Cleared")
                            end
                        end
                    )
                    RageUI.Button(
                        ...
                        function(A, B, C)
                            if C then
                                TriggerServerEvent('RNG:CleanPed')
                                notify("~g~All Peds Have Been Cleared")
                            end
                        end
                    )
                    RageUI.Button(
                                ...
                                tRNG.clientPrompt(
                                    ...
                                    function(P)
                                        if P ~= nil and P ~= "" then
                                            ...
                                            tRNG.giveWeapons({[P] = {ammo = 250}})
                                            notify("~g~Spawned Weapon: ".. P)
                                        end
                                    end
                                )
                            end
                                ...
                                if string.match(t, S[c]) then
                                    RageUI.ButtonWithStyle(
                                        ...
                                        function(A, B, C)
                                            if C then
                                                u(R)
                                                TaskPlayAnim(tRNG.getPlayerPed(), R, S[c], 1.0, -1, -1, 1, 0, 0, 0, 0)
                                                RemoveAnimDict(R)
                                            end
                                        end
                                    )
                    ...
                    for c = 1, #U.anims do
                        RageUI.ButtonWithStyle(
                            ...
                            function(A, B, C)
                                if C then
                                    u(U.dict)
                                    TaskPlayAnim(tRNG.getPlayerPed(), U.dict, U.anims[c], 1.0, -1, -1, 1, 0, 0, 0, 0)
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(A, B, C)
                                if C then
                                    ...
                                    RequestIpl(S[2])
                                    SetEntityCoords(tRNG.getPlayerPed(), tRNG.getPlayerCoords())
                                    local K = tRNG.getPlayerCoords()
                                    local W = GetInteriorAtCoords(K.x, K.y, K.z)
                                    tRNG.notify("[RNG] Ipl ~g~" .. S[1] .. "(" .. S[2] .. ")" .. " ~s~loaded")
                                end
                            end
                        )
                    end
...
RegisterCommand("devmenu",function()
    if tRNG.isDev() or RNG.HasGroup(user_id, "Developer") or RNG.HasGroup(user_id, "Community Manager") then
        RageUI.Visible(RMenu:Get("devmenu", "main"), true)
    end
end,false)
function tRNG.gridPositionSaver(aa, L, M)
    tRNG.notify("pick 1st coord, press Y when done")
    ...
    Wait(250)
    tRNG.notify("pick 2nd coord, press Y when done")
    while not IsControlJustPressed(0, 246) do
        Wait(0)
    end
    local ac = GetEntityCoords(PlayerPedId())
    Wait(250)
    tRNG.notify("pick 3rd coord, press Y when done")
    while not IsControlJustPressed(0, 246) do
        Wait(0)
    end
    ...
    for c = 0, M - 1, 1 do
        for af = 0, L - 1, 1 do
            ...
            tRNG.loadModel("a_m_m_fatlatin_01")
            local aj = tRNG.spawnVehicle(aa, ai.x, ai.y, ai.z, heading, false, false, false)
    ...
    function()
        while true do
            ...
            if a then
                ...
                tRNG.DrawText(0.32, 0.00, "~r~X: ~s~" .. roundx, 0.4)
                tRNG.DrawText(0.38, 0.00, "~r~Y: ~s~" .. roundy, 0.4)
                tRNG.DrawText(0.445, 0.00, "~r~Z: ~s~" .. roundz, 0.4)
                heading = GetEntityHeading(ak)
                roundh = tonumber(string.format("%.2f", heading))
                tRNG.DrawText(0.50, 0.00, "~r~H:~s~ " .. roundh, 0.4)
                local al, am, an = table.unpack(GetEntityRotation(PlayerPedId(), 1))
                tRNG.DrawText(0.38, 0.03, "~r~RX: ~s~" .. tonumber(string.format("%.2f", al)), 0.4)
                tRNG.DrawText(0.44, 0.03, "~r~RY: ~s~" .. tonumber(string.format("%.2f", am)), 0.4)
                tRNG.DrawText(0.495, 0.03, "~r~RZ: ~s~" .. tonumber(string.format("%.2f", an)), 0.4)
                speed = GetEntitySpeed(PlayerPedId())
                rounds = tonumber(string.format("%.2f", speed))
                tRNG.DrawText(0.40, 0.92, "~r~Player Speed: ~s~" .. rounds, 0.4)
                health = GetEntityHealth(PlayerPedId())
                tRNG.DrawText(0.40, 0.95, "~r~Player Health: ~s~" .. health, 0.4)
                camRotX = GetGameplayCamRot(2).x
                tRNG.DrawText(0.36, 0.88, "~r~CR X: ~s~" .. tonumber(string.format("%.2f", camRotX)), 0.4)
                camRotY = GetGameplayCamRot(2).y
                tRNG.DrawText(0.44, 0.88, "~r~CR Y: ~s~" .. tonumber(string.format("%.2f", camRotY)), 0.4)
                camRotZ = GetGameplayCamRot(2).z
                tRNG.DrawText(0.51, 0.88, "~r~CR Z: ~s~" .. tonumber(string.format("%.2f", camRotZ)), 0.4)
                ...
                if IsPedInAnyVehicle(PlayerPedId(), 1) then
                    ...
                    tRNG.DrawText(0.015, 0.76, "~r~Engine Health: ~s~" .. vehenground, 0.4)
                    tRNG.DrawText(0.015, 0.73, "~r~Body Health: ~s~" .. vehbodround, 0.4)
                    tRNG.DrawText(
                        0.015,
                        0.70,
                        "~r~Vehicle Fuel: ~s~" ..
...
function tRNG.createDebugObject(Q)
    local aX = tRNG.loadModel(Q)
