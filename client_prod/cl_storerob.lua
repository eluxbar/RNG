...
AddEventHandler(
    ...
    function(t, u)
        if u then
            TriggerServerEvent("RNG:getStoreRobBlips")
            while p == nil do
                Wait(0)
            end
            while true do
    ...
    if not IsPedDeadOrDying(x) then
        if IsEntityPlayingAnim(x, "mp_am_hold_up", "holdup_victim_20s", 3) then
            ...
            if not IsPedDeadOrDying(x) then
                ...
                local A = tRNG.loadModel("prop_poly_bag_01")
                        ...
                        function()
                            while true do
                                ...
                                if DoesEntityExist(B) then
                                    if #(GetEntityCoords(tRNG.getPlayerPed()) - GetEntityCoords(B)) <= 1.5 then
                                        PlaySoundFrontend(
                                            -1,
                                            "ROBBERY_MONEY_TOTAL",
                                        ...
                                        TriggerServerEvent("RNG:pickupCashFromApu", v)
                                        DeleteObject(B)
                                        break
                                    end
                                else
...
Citizen.CreateThread(
    function()
        while true do
            local t = GetEntityCoords(tRNG.getPlayerPed(), true)
            if r ~= nil then
                for v, w in pairs(r) do
...
function refreshStoreBlips(u)
    ...
    for D, E in pairs(p) do
        ...
        if E.beingrobbed == true then
            tempBlip = tRNG.addBlip(F.x, F.y, F.z, 52, 1, "Robbable Store [BEING ROBBED]")
        else
            tempBlip = tRNG.addBlip(F.x, F.y, F.z, 52, 2, "Robbable Store")
        end
...
Citizen.CreateThread(
    function()
        for v, w in pairs(r) do
            local G = tRNG.loadModel(w.shopNpcModel)
            local H =
                CreatePed(
                26,
                G,
            ...
            SetPedFleeAttributes(H, 0, 0)
            local I = tRNG.loadModel(w.prop_safe)
            local J = tRNG.loadModel(w.prop_door)
            local K = tRNG.loadModel(w.money_prop)
            local L = tRNG.loadModel(w.money_prop2)
            local M = tRNG.loadModel(w.money_prop3)
            local N = tRNG.loadModel(w.money_prop4)
        ...
        while true do
            local U = GetEntityCoords(tRNG.getPlayerPed())
            for v, w in pairs(r) do
                             ...
                             then
                                if not q then
                                    if GetSelectedPedWeapon(tRNG.getPlayerPed()) == GetHashKey("WEAPON_UNARMED") then
                                        tRNG.notify("~r~You need a weapon in your hands to rob this store!")
                                    else
                                        TriggerServerEvent("RNG:initiateStoreRobbery", v)
...
RegisterNetEvent(
    "RNG:beginStoreRobbingAnimations",
    function(v)
...
RegisterNetEvent(
    "RNG:storeRobberyInProgress",
    function(Y, v)
        q = Y
        local Z = GetEntityCoords(tRNG.getPlayerPed())
        while q do
            local U = GetEntityCoords(tRNG.getPlayerPed())
            if #(Z - U) > 20 then
                TriggerServerEvent("RNG:forceEndRobbery", v)
...
RegisterNetEvent(
    ...
    function(v)
        ...
        local G = tRNG.loadModel(r[v].shopNpcModel)
        local H =
            CreatePed(
            26,
            G,
...
function RunMiniGame(v, ab, ac)
    local ad = 1
    local a = getRandomSafeCombination(1)
    ...
    while safeCrackingInProgress do
        ...
        elseif safeCrackingState == "cracking" then
            ...
            local ae = GetEntityHealth(tRNG.getPlayerPed())
            if ae <= 102 or tRNG.isHandcuffed() then
                ...
                if IsControlJustPressed(0, 172) then
                    if af == a[ad] then
                        ...
                        if IsSafeUnlocked(b, ad) then
                            ...
                            safeCrackingInProgress = false
                            ClearPedTasksImmediately(tRNG.getPlayerPed())
                        end
                    else
                        ad = 1
                        b = InitSafeLocks(a)
                        a = getRandomSafeCombination(1)
                        PlaySoundFrontend(0, "TUMBLER_RESET", "SAFE_CRACK_SOUNDSET", true)
...
function GetSafeDoorAnimOffsetPosition(ah, ai, aj)
    ...
    return vector3(ah.x + ao, ah.y + ap, GetEntityCoords(tRNG.getPlayerPed()).z)
end
function PlaySafeCrackIntroAnim(aq, h)
    local ar = "mini@safe_cracking"
    local as = "step_into"
    loadAnimDict(ar)
    TaskPlayAnimAdvanced(tRNG.getPlayerPed(), ar, as, aq.x, aq.y, aq.z, 0.0, 0.0, h, 8.0, 8.0, -1, 2, 0.7, 0, 0)
    Wait(0)
    Wait(1000)
    safeCrackingState = "cracking"
end
function HandleSafeDialMovement()
    local at = tRNG.getPlayerPed()
    local ar = "mini@safe_cracking"
    local au = ""
    ...
    loadAnimDict(ar)
    if not IsEntityPlayingAnim(tRNG.getPlayerPed(), ar, au, 3) then
        TaskPlayAnimAdvanced(tRNG.getPlayerPed(), ar, au, g.x, g.y, g.z, 0.0, 0.0, h, 8.0, 8.0, -1, 1, 1.0, 0, 0)
    end
end
function HandleIncorrectMovement(ad, b)
    ...
    loadAnimDict(ar)
    TaskPlayAnimAdvanced(tRNG.getPlayerPed(), ar, ax, g.x, g.y, g.z, 0.0, 0.0, h, 8.0, 8.0, -1, 1, 1.0, 0, 0)
    d = "rotation.idle"
end
...
function EndMiniGame(v, ab)
    ...
    Wait(2500)
    TriggerServerEvent("RNG:syncOpenSafeDoor", v)
    safeCrackingState = "setup"
