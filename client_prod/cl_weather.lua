...
function tRNG.setTime(f, g, h)
    b = {h = f, m = g, s = h}
end
function tRNG.overrideTime(f, g, h)
    b.h = f
    b.m = g
    b.s = h
    c = true
end
function tRNG.cancelOverrideTimeWeather()
    c = false
end
function tRNG.setWeather(i)
    a = i
end
...
tRNG.createThreadOnTick(func_manageTimeAndWeather)
RegisterNetEvent(
    "RNG:setTime",
    function(f, g, h)
        tRNG.setTime(f, g, h)
...
function voteWeather(C)
    if j then
        if not k then
            TriggerServerEvent("RNG:vote", C)
            k = true
            tRNG.notify("~g~Vote sent!")
        else
            tRNG.notify("You have already voted!")
        end
    else
        tRNG.notify("Vote not in progress, start a vote with /voteweather!")
    end
end
function resetVotes()
    m = {
...
RegisterCommand(
    "voteweather",
    function()
        TriggerServerEvent("RNG:tryStartWeatherVote")
    end,
    false
)
RegisterCommand(
