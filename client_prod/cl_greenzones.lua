local a = module("rng-vehicles", "cfg/cfg_vehiclemaxspeeds")
isInGreenzone = false
local b = true
local c = false
local d = false
...
local m = {
    ...
    },
    {
        colour = 2,
        id = 1,
        pos = vector3(-253.40475463867,6309.4467773438,32.420124053955),
        dist = 40,
        nonRP = false,
        setBit = false,
        maxHeight = 105.0
    },
    {
        colour = 2,
        id = 1,
        pos = vector3(1836.8889160156,3685.6384277344,34.262802124023),
        dist = 40,
        nonRP = false,
        setBit = false,
        maxHeight = 105.0
    ...
    },
    {
        colour = 2,
        id = 1,
        pos = vector3(-2188.101807, 5187.242676, 17.242422),
        dist = 40,
        nonRP = false,
        setBit = false,
        maxHeight = 87.0
...
local n = {
    ...
    {vector3(-253.40475463867,6309.4467773438,32.420124053955), 40.0, 2, 180}, -- paleto hospital
    {vector3(1836.8889160156,3685.6384277344,34.262802124023), 40.0, 2, 180}, -- Sandy hospital
    {vector3(148.066, -1050.763, 29.34099), 40.0, 2, 180},
    {vector3(-2188.101807, 5187.242676, 17.242422), 120.0, 2, 180},
    ...
    {vector3(1465.1910400391,6326.611328125,19.009782791138), 100.0, 1, 180}, -- REBEL
    {vector3(1249.615845, 7062.604980, 15.337686), 175.0, 1, 180}, -- Gang Island
    ...
    --{vector3(1791.5841064453,4598.9921875,38.500267028809), 20.0, 2, 180} -- Old rebel spawn location
}
...
function tRNG.areGreenzonesDisabled()
    return j
end
function tRNG.setGreenzonesDisabled(q)
    j = q
end
o(
    function()
        for r, s in pairs(n) do
            if not s[5] and not tRNG.isPurge() or s[5] and tRNG.isPurge() then
                Wait(500)
...
o(
    function()
        local u = tRNG.isPurge()
        while true do
            local v = tRNG.getPlayerPed()
            local w = tRNG.getPlayerCoords()
            for x, y in pairs(m) do
                if not y.isPurge and not u or y.isPurge and u then
                    ...
                    SetPlayerInvincible(tRNG.getPlayerId(), false)
                    ...
                    SetNetworkVehicleAsGhost(tRNG.getPlayerVehicle(), false)
                end
            end
            p(250)
        end
...
local function G()
    ...
    if I then
        local M, N = tRNG.getPlayerVehicle()
        if M ~= 0 then
        ...
        while true do
            ...
            if not tRNG.areGreenzonesDisabled() then
                ...
                if c or d then
                    ...
                    ClearPedBloodDamage(v)
                    if usingDelgun then
                        tRNG.setWeapon(v, "WEAPON_STAFFGUN", true)
                    else
                ...
                if c then
                    ...
                    local W, X = tRNG.getPlayerCombatTimer()
                    if W > 0 and X then
                        G()
                    end
                end
...
RegisterCommand(
    "togglecitycap",
    function()
        if tRNG.getUserId() == 1 or tRNG.getUserId() == 0 then
            b = not b
            tRNG.notify("City Cap: " .. (b and "~g~Enabled" or "~r~Disabled"))
        end
    end
)
