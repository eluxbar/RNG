...
RageUI.CreateWhile(
    ...
    true,
    function()
        if RageUI.Visible(RMenu:Get("frscenemenu", "main")) then
            RageUI.DrawContent(
                ...
                function()
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        RMenu:Get("frscenemenu", "speedZone")
                    )
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(J, K, L)
                            if L then
                                RageUI.Visible(RMenu:Get("frscenemenu", "main"), false)
                            end
                        end
                    )
                end
        ...
        if RageUI.Visible(RMenu:Get("frscenemenu", "objects")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = false},
                function()
                    RageUI.List(
        ...
        if RageUI.Visible(RMenu:Get("frscenemenu", "speedZone")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = false},
                function()
                    RageUI.List(
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(J, K, L)
                            if L then
                                if o == 0 then
                                    tRNG.notify("Please set a radius")
                                elseif p == 0 then
                                    tRNG.notify("Please set a speed")
                                else
                                    TriggerServerEvent("RNG:createSpeedZone", tRNG.getPlayerCoords(), p + 0.0, o + 0.0)
                                    tRNG.notify("~b~Speed zone created")
                                end
                            end
                        end,
                        nil
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(J, K, L)
                            if L then
                                ...
                                if Q then
                                    TriggerServerEvent("RNG:deleteSpeedZone", Q)
                                    tRNG.notify("Speed zone removed")
                                end
                            end
                        end,
                        nil
        ...
        if RageUI.Visible(RMenu:Get("frscenemenu", "spikes")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = false},
                function()
                    RageUI.ButtonWithStyle(
...
RegisterNetEvent(
    "RNG:toggleTrafficMenu",
    function()
        RageUI.Visible(RMenu:Get("frscenemenu", "main"), not RageUI.Visible(RMenu:Get("frscenemenu", "main")))
        if globalOnPoliceDuty and tRNG.hasPoliceCallsign() then
            RMenu:Get("frscenemenu", "main"):SetSubtitle(
                "~b~MPD~w~: " ..
                    tRNG.getPoliceCallsign() .. " - " .. tRNG.getPoliceRank() .. " - " .. tRNG.getPoliceName()
            )
        end
    end
)
function deleteObject()
    for m, n in pairs(j) do
        ...
        local X, Y, Z = table.unpack(GetEntityCoords(tRNG.getPlayerPed(), true))
        if DoesObjectOfTypeExistAtCoords(X, Y, Z, 0.9, W, true) then
            ...
            if NetworkGetEntityIsNetworked(_) and not NetworkHasControlOfEntity(_) then
                local a0 = tRNG.getNetId(_, "deleteObject()")
                if a0 ~= 0 then
                    TriggerServerEvent("RNG:requestSceneObjectDelete", a0)
                end
            else
                DeleteObject(_)
            end
            ...
            for m, a1 in pairs(a) do
                if DoesEntityExist(a1) then
                    if NetworkGetEntityIsNetworked(a1) then
                        ...
                        if a2 ~= 0 then
                            TriggerServerEvent("RNG:requestSceneObjectDelete", a2)
                        end
                    else
                        DeleteEntity(a1)
                    end
...
function placeSpike()
    local a3 = tRNG.getPlayerVehicle()
    if a3 == 0 then
        local a4 = "P_ld_stinger_s"
        local a5 = tRNG.getPlayerPed()
        ...
        TriggerServerEvent("RNG:placeSpike", a6, P)
    end
end
...
function deleteSpike()
    local P = GetEntityCoords(tRNG.getPlayerPed())
    local a7 = getClosetSpike(P)
    if a7 then
        TriggerServerEvent("RNG:removeSpike", a7[2])
    end
end
RegisterNetEvent(
    "RNG:addSpike",
...
Citizen.CreateThread(
    function()
        while true do
            local a3, af = tRNG.getPlayerVehicle()
            if a3 ~= 0 and af then
                local ag = false
                local ah = 1000.0
