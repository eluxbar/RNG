RMenu.Add(
    "nhs",
    "main",
    RageUI.CreateMenu("NHS", "Status: ~b~Available", tRNG.getRageUIMenuWidth(), tRNG.getRageUIMenuHeight())
)
RMenu.Add(
    "nhs",
    "equipment",
                    ...
                    if e.active then
                        ...
                        if e.readyToRevive then
                            RageUI.Button(
                                ...
                                function(g, h, i)
                                    if i then
                                        TriggerServerEvent("RNG:finishRevive", e.perm_id)
                                        e.readyToRevive = false
                                        TriggerServerEvent("RNG:playNhsSound", "shockAdvisedCharging")
                                    end
                                end
                            )
                        else
                            RageUI.Button(
                                ...
                                function(g, h, i)
                                    if i then
                                        ...
                                        TriggerServerEvent("RNG:playNhsSound", "threeBeeps")
                                    end
                                end
                            )
                        end
            ...
            RageUI.DrawContent(
                ...
                function()
                            ...
                            RageUI.Button(
                                ...
                                function(g, h, i)
                                    if i then
                                        TriggerServerEvent("RNG:finishRevive", e.perm_id)
                                        e.readyToRevive = false
                                        TriggerServerEvent("RNG:playNhsSound", "carryOutShock")
                                    end
                                end
                            )
                        end
                    ...
                    RageUI.Button(
                        ...
                        function(g, h, i)
                            if i then
                                TriggerServerEvent("RNG:playNhsSound", "metronome")
                            end
                        end
                    )
...
function carryLifePak()
    ...
    f.active = true
    local l = tRNG.loadModel("prop_lifepak")
    ...
    Citizen.CreateThread(
        function()
            ...
            if not e.dead then
                ...
                if n == 3 then
                    Wait(3000)
                    TriggerServerEvent("RNG:playNhsSound", "checkPatientAlarm")
                end
...
function beginObs(a, u, v, w, d)
    TriggerServerEvent("RNG:playNhsSound", "threeBeeps")
    Wait(3000)
    TriggerServerEvent("RNG:playNhsSound", "analysingHeartRythm")
    e = {}
    ...
    if e.coma then
        ...
        if IsEntityDead(GetPlayerPed(u)) then
            ...
            if e.readyToRevive then
                ...
                TriggerServerEvent("RNG:playNhsSound", "pushToShock")
            end
        end
    end
end
function connectObsToPatient()
    TriggerServerEvent("RNG:attachLifepakServer")
end
RegisterNetEvent(
    "RNG:clientPlayNhsSound",
    function(x, y)
...
RegisterNetEvent(
    ...
    function()
        ...
        e.coma = false
        Wait(3000)
        TriggerServerEvent("RNG:playNhsSound", "safeToTouch")
        Wait(12000)
        TriggerServerEvent("RNG:playNhsSound", "threeBeeps")
    end
)
    ...
    if globalNHSOnDuty or globalLFBOnDuty then
        ...
        if E ~= GetHashKey("WEAPON_UNARMED") then
            if not table.has(B, E) then
                tRNG.setWeapon(D, "WEAPON_UNARMED", true)
            end
        end
    end
end
tRNG.createThreadOnTick(C)
local F = false
RegisterNetEvent(
    "RNG:attemptCPR",
    function()
        F = true
        tRNG.loadAnimDict("missheistfbi3b_ig8_2")
        TaskPlayAnim(
            PlayerPedId(),
        ...
        while F do
            if not IsEntityPlayingAnim(PlayerPedId(), "missheistfbi3b_ig8_2", "cpr_loop_paramedic", 3) then
                F = false
                TriggerServerEvent("RNG:cancelCPRAttempt")
                break
            end
            Citizen.Wait(0)
        end
...
RegisterNetEvent(
    "RNG:cancelCPRAttempt",
    function()
        F = false
        if tRNG.canAnim() then
