...
Citizen.CreateThread(
    function()
        for d, e in pairs(a.peds) do
            tRNG.createDynamicPed(e.modelHash, e.position, e.heading, true, e.animDict, e.animName, 30, false, false)
            tRNG.addBlip(e.position.x, e.position.y, e.position.z, 357, 81, "Vehicle Impound", 0.8, false)
        end
        Wait(2000)
        local f = function()
            TriggerServerEvent("RNG:getImpoundedVehicles")
            RageUI.Visible(RMenu:Get("frimpound", "main"), true)
        end
        local g = function()
        ...
        for d, e in pairs(a.peds) do
            tRNG.createArea("vehicle_impound_" .. d, e.position, 3.0, 6, f, g, h, {})
        end
    end
)
RMenu.Add(
...
RMenu.Add("frimpound", "vehicle_information", RageUI.CreateSubMenu(RMenu:Get("frimpound", "main")))
RMenu.Add("frimpound", "pay_fine_confirmation", RageUI.CreateSubMenu(RMenu:Get("frimpound", "vehicle_information")))
local i = 0
RageUI.CreateWhile(
    1.0,
    true,
                ...
                if c ~= nil and c ~= {} then
                    for p, q in pairs(c) do
                        RageUI.ButtonWithStyle(
                            ...
                            RMenu:Get("frimpound", "vehicle_information")
                        )
                    end
                else
            ...
            function()
                if i ~= 0 then
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        RMenu:Get("frimpound", "pay_fine_confirmation")
                    )
                end
            end,
            function()
            ...
            function()
                if i ~= 0 then
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(k, d, l)
                            if l then
                                TriggerServerEvent("RNG:releaseImpoundedVehicle", i.vehicle)
                            end
                        end,
                        RMenu:Get("frimpound", "main")
                    )
...
RegisterNetEvent(
    ...
    function(x, u, y, z, A, B, C, D)
        ...
        if O ~= vector3(0, 0, 0) and N ~= vector3(0, 0, 0) then
            tRNG.loadModel("flatbed")
            tRNG.loadModel("a_m_m_prolhost_01")
            P = tRNG.spawnVehicle("flatbed", O.x, O.y, O.z, M, false, true, true)
            ...
            TriggerServerEvent(
                "RNG:deleteImpoundEntities",
                x,
                NetworkGetNetworkIdFromEntity(G),
                NetworkGetNetworkIdFromEntity(F)
            )
            v = tRNG.spawnVehicle(z, C.x, C.y, C.z, D, false, true, false)
            ...
            TriggerServerEvent("RNG:awaitTowTruckArrival", x, R, T)
...
function tRNG.impoundVehicleOptions(Z, _, x, a0)
    ...
    elseif a.disallowedVehicleClasses[a2] then
        ...
        TriggerServerEvent("RNG:fetchInfoForVehicleToImpound", Z, a1, x)
        RageUI.Visible(RMenu:Get("frimpound", "reasons"), true)
    end
end
function tRNG.isVehicleImpounded(v)
    return c[v] ~= nil
end
...
exports("impound", tRNG.impoundVehicleOptions)
exports("isImpounded", tRNG.isVehicleImpounded)
