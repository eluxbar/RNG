...
AddEventHandler(
    ...
    function(p, q)
        if q then
            tRNG.addMarker(b.x, b.y, b.z, 0.7, 0.7, 0.5, 0, 125, 255, 125, 150, 20, false, false, true)
            tRNG.addBlip(b.x, b.y, b, 378, 3, "Organ Heist Police")
            tRNG.addMarker(c.x, c.y, c.z, 0.7, 0.7, 0.5, 255, 0, 0, 125, 150, 20, false, false, true)
            tRNG.addBlip(c.x, c.y, c.z, 378, 1, "Organ Heist Criminals")
        end
    end
)
    ...
    function()
        while true do
            if f then
                ...
                if IsControlJustPressed(0, 38) then
                    if not globalOnPoliceDuty then
                        if not g then
                            TriggerServerEvent("RNG:joinOrganHeist")
                            Wait(100)
                        else
                            tRNG.notify("~r~Already joined the Organ Heist!")
                        end
                    else
                        tRNG.notify("~r~You are police, please use the other entrance!")
                    end
                end
            elseif e then
                ...
                if IsControlJustPressed(0, 38) then
                    if globalOnPoliceDuty then
                        ...
                        tRNG.notify("~r~You are a civillian, please use the other entrance!")
                    end
                end
            end
            ...
            if inGamePhase or inWaitingStage then
                ...
                RemoveWeaponFromPed(tRNG.getPlayerPed(), "WEAPON_MOLOTOV")
                RemoveWeaponFromPed(tRNG.getPlayerPed(), "WEAPON_FLASHBANG")
                RemoveWeaponFromPed(tRNG.getPlayerPed(), "WEAPON_SMOKEGRENADEFR")
                RemoveWeaponFromPed(tRNG.getPlayerPed(), "WEAPON_SMOKEGRENADEFRPD")
                RemoveWeaponFromPed(tRNG.getPlayerPed(), "WEAPON_BZGAS")
                RemoveWeaponFromPed(tRNG.getPlayerPed(), "WEAPON_STUNGUN")
                local r = PlayerPedId()
...
RegisterNetEvent(
    "RNG:organHeistKillConfirmed",
    function(C)
        PlaySoundFrontend(-1, "Weapon_Upgrade", "DLC_GR_Weapon_Upgrade_Soundset", true)
        tRNG.playScreenEffect("MP_Celeb_Win", 0.25)
        tRNG.notify("~g~Killed " .. C .. " received £100,000 and 25% Armour")
    end
)
local function D()
    local E = 1000.0
        ...
        while true do
            if inWaitingStage then
                ...
                if E < 3.0 then
                    ...
                    NetworkFadeInEntity(PlayerPedId(), 0)
                    DoScreenFadeIn(1000)
                    tRNG.notify("~r~You got too far from the organ heist and have been teleported back.")
                end
            end
            if inGamePhase then
                local l, F = D()
...
RegisterNetEvent(
    ...
    function()
        ...
        inGamePhase = false
        d = 600
        tRNG.deleteAtm("Organ Heist")
        for l, A in pairs(k.gunStores[h]) do
            tRNG.deleteGunStore(A[1])
        end
        h = ""
        k = nil
...
function tRNG.setDeathInOrganHeist()
    inOrganHeist = false
    inWaitingStage = false
    inGamePhase = false
end
