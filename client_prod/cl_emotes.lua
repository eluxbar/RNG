...
local function p(q)
    ...
    if not q and GetGameTimer() - e < 1500 then
        tRNG.notify("Emotes are being rate limited.")
        return
    end
    b = false
    f = nil
...
local function F(E)
    if tRNG.getPlayerVehicle() ~= 0 then
        tRNG.notify("Can not use scenarios whilst in a vehicle.")
        return
    end
    local A = PlayerPedId()
    ...
    elseif E[1] == "MaleScenario" then
        if tRNG.getModelGender() == "male" then
            ...
            tRNG.notify("This scenario is male only.")
        end
...
local function a7(E)
    if not tRNG.canAnim() or tRNG.isHandcuffed() then
        return true
    end
    local A = PlayerPedId()
    ...
    local a8 = tRNG.getPlayerVehicle()
...
local function a6(E)
    if a7(E) then
        tRNG.notify("Can not this emote at this time.")
        return
    end
...
local function ag(ah)
    if a7() then
        tRNG.notify("Can not use shared emotes at this time.")
        return
    end
    local ai = aa()
    if ai ~= 0 then
        i = ah
        j = ai
        TriggerServerEvent("RNG:sendSharedEmoteRequest", ai, ah)
    else
        tRNG.notify("No player is near by.")
    end
end
local function aj(E)
    ...
    function()
        ...
        if RageUI.Visible(RMenu:Get("emotesmenu", "searchemotes")) then
            RageUI.DrawContent(
                ...
                function()
                    if az == false then
                        ay = tRNG.KeyboardInput("Enter Emote Name", "", 10)
                        if ay == nil then
                            ay = ""
                        end
                    end
...
local function aH(aE, aF, aG)
    if #aF < 1 then
        tRNG.notify("No emote name was specified.")
        return
    end
    ...
    elseif a.guns[ah] then
        ...
        tRNG.notify("~r~Invalid emote name was specified.")
    end
end
...
RegisterCommand(
    ...
    function(aE, aF, aG)
        if #aF < 1 then
            tRNG.notify("No walk name was specified.")
            return
        end
        local ah = aF[1]
        if not ah then
        ...
        if a.walks[ah] then
            ...
            tRNG.notify("~r~Invalid walk name was specified.")
        end
    end
)
RegisterNetEvent(
    "RNG:sendSharedEmoteRequest",
    function(aI, ah)
        if a.shared[ah] and not a7() then
            k = ah
            l = aI
            m = GetGameTimer()
            tRNG.notify("~y~Y~w~ to accept, ~r~L~w~ to refuse (~g~" .. a.shared[ah][3] .. "~w~)")
        end
    end
)
RegisterNetEvent(
    "RNG:receiveSharedEmoteRequest",
    function(ah)
        p(false)
        Citizen.Wait(300)
        a6(a.shared[ah])
...
RegisterNetEvent(
    "RNG:receiveSharedEmoteRequestSource",
    function()
...
local function aV()
    if k then
        ...
        elseif IsControlJustPressed(1, 246) then
            if a7() then
                tRNG.notify("You can not use emotes at this time.")
            else
                TriggerServerEvent("RNG:receiveSharedEmoteRequest", l, a.shared[k][4])
            end
            aM()
        end
    end
    if b and f and g then
        ...
        if not IsEntityPlayingAnim(PlayerPedId(), f, g, 3) or tRNG.getPlayerCombatTimer() > 0 then
            p(true)
        end
    end
end
function tRNG.playEmote(ah)
    if a.emotes[ah] then
        a6(a.emotes[ah])
    elseif a.dances[ah] then
        a6(a.dances[ah])
