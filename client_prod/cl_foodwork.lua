        ...
        if f then
            ...
            local i = function(j)
                if not c and IsControlJustPressed(1, 51) then
                    TriggerServerEvent("RNG:requestStartCooking", j.recipe)
...
local function t()
    for p, q in pairs(b) do
        for r, s in pairs(q) do
            local u = tRNG.loadModel(s.model)
    ...
    function(e, f)
        if f then
            ...
            for w, x in pairs(b) do
                tRNG.createArea("cooking_staticobjects_" .. w, x[1].position, 100, 5, g, h, i, {})
            end
        end
    end
)
...
local function D(E)
    local F = true
    local G
    tRNG.minigameCircularProgressBar(
...
function cookBBQ()
    ...
    while L < 3 do
        ...
        if H then
            ...
            tRNG.announceMpSmallMsg("Disaster", "You dropped the burger patty, start again!", 6, 3000)
        end
        Wait(2500)
        tRNG.startCircularProgressBar(
            "",
    ...
    tRNG.announceMpSmallMsg("Success", "You made a burger!", 18, 3000)
    ClearPedTasks(C)
    StopParticleFxLooped(K, true)
end
local function M()
    local C = tRNG.getPlayerPed()
    local N = tRNG.getPlayerCoords()
    local u = tRNG.loadModel("prop_food_bs_tray_02")
    ...
    SetModelAsNoLongerNeeded(u)
    tRNG.loadAnimDict("anim@heists@box_carry@")
    TaskPlayAnim(C, "anim@heists@box_carry@", "idle", 2.0, 2.0, -1, 51, 0, false, false, false)
    Citizen.CreateThread(
        function()
            while tRNG.doesAreaExist("temp_foodorder_complete") do
...
local function T(U)
    ...
    local W = tRNG.addMarker(V.x, V.y, V.z + 0.3, 0.15, 0.15, 0.15, 255, 215, 0, 170, 50, 0, false, true, true)
    local X = tRNG.addBlip(V.x, V.y, V.z, 1, 2, "Delivery Point", 0.6)
    ...
    tRNG.createArea(
        ...
        6.0,
        function()
            tRNG.removeArea("temp_foodorder_complete")
            tRNG.removeMarker(W)
            tRNG.removeBlip(X)
...
RegisterNetEvent(
    "RNG:cookingInstruction",
    function(Z)
        for r, _ in pairs(R) do
            tRNG.removeArea("temp_cooking_instruction")
            tRNG.removeMarker(_)
        end
        for r, Y in pairs(S) do
            tRNG.removeBlip(Y)
        ...
        local W = tRNG.addMarker(V.x, V.y, V.z + 0.3, 0.15, 0.15, 0.15, 255, 215, 0, 170, 50, 0, false, true, true)
        table.insert(R, W)
        local X = tRNG.addBlip(V.x, V.y, V.z, 1, 2, "Ingredient", 0.6)
        SetBlipFlashes(X, true)
        table.insert(S, X)
        local g = function()
        end
        ...
        local i = function()
            if IsControlJustPressed(1, 51) then
                ...
                if Z == "bbq" then
                    ...
                    T(
                        function()
                            TriggerEvent("RNG:playItemBoughtSound")
                            TriggerServerEvent("RNG:pickupCookingIngredient", c, Z)
                            tRNG.setCanAnim(true)
                            tRNG.announceMpSmallMsg("Success", "You earned £4,000, now get back to work!", 18, 3000)
                        end
                    )
                else
                    tRNG.removeMarker(W)
        ...
        tRNG.createArea("temp_cooking_instruction", V, 1.0, 6, g, h, i)
    end
)
RegisterNetEvent(
    "RNG:beginCooking",
    function(a0)
        TriggerEvent("RNG:smallAnnouncement", "Cooking", "Make a " .. a0 .. ".", 15, 5000)
    end
)
RegisterNetEvent(
    "RNG:finishedCooking",
