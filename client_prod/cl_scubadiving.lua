...
RMenu.Add(
    "scubadiving",
    "main",
    RageUI.CreateMenu("", "", tRNG.getRageUIMenuWidth(), tRNG.getRageUIMenuHeight(), "rng_scubajob", "rng_scubajob")
)
RMenu:Get("scubadiving", "main"):SetSubtitle("RNG | ~b~Scuba Diving")
local function j()
...
AddEventHandler(
    ...
    function(l, m)
        if m then
            ...
            tRNG.addBlip(a.jobPosition.x, a.jobPosition.y, a.jobPosition.z, 308, 0, "Scuba Diving")
            tRNG.addMarker(
                a.jobPosition.x,
                a.jobPosition.y,
                a.jobPosition.z - 0.2,
            ...
            tRNG.createArea("scubajob", a.jobPosition, 1.5, 6, q, s, t, {})
        end
    end
)
RageUI.CreateWhile(
                ...
                function()
                    if not e then
                        RageUI.Button(
                            ...
                            function(u, v, w)
                                if w then
                                    TriggerServerEvent("RNG:requestScubaJob")
                                end
                            end
                        )
                    end
                    if d ~= 0 then
                        RageUI.Button(
                            ...
                            function(u, v, w)
                                if w then
                                    TriggerServerEvent("RNG:claimScubaReward", c)
                                    d = 0
                                    c = nil
                                    e = false
                                    i = false
                                    ...
                                    tRNG.setCustomization(h)
                                    local x = PlayerPedId()
                                    SetEnableScuba(x, false)
                                    SetPedMaxTimeUnderwater(x, 10.0)
                                    tRNG.setMaxUnderWaterUITimenewTime(10.0)
                                end
                            end
                        )
                    end
...
local function y()
    for z, A in pairs(b) do
        tRNG.removeMarker(A.objectMarker)
        tRNG.removeArea(A.objectArea)
...
function startScubaDiving()
    tRNG.loadAnimDict("missheistchem2")
    local x = PlayerPedId()
    ...
    tRNG.drawNativeNotification("Press ~INPUT_PICKUP~ to start Scuba Diving.")
    while not F do
    ...
    TriggerServerEvent("RNG:finishScubaJob", c, d)
end
RegisterNetEvent(
    "RNG:createScubaJob",
    function(J, K)
        c = K
        e = true
        tRNG.disableCustomizationSave(e)
        DoScreenFadeOut(500)
        Wait(600)
        ...
        SetBlipAlpha(L, 180)
        local q = function(r)
            tRNG.drawNativeNotification("Press ~INPUT_DETONATE~ to begin diving!")
        end
        local s = function(r)
        end
        local t = function(r)
            if IsControlJustReleased(1, 47) then
                local x = tRNG.getPlayerPed()
                local M = tRNG.getPlayerVehicle()
                if M ~= 0 then
                    if GetEntityModel(M) ~= GetHashKey("dinghy") then
                        tRNG.notify("You must be in a dinghy to start scuba diving!")
                    else
                        tRNG.removeArea("scubajob_" .. J.name)
                        f = "~y~Search for treasures and return to your boat when finished"
                        createRewardObjects(J)
                        CreateThread(startScubaDiving)
                    end
                else
                    tRNG.notify("You must be in a boat to start scuba diving!")
                end
            end
        end
        SetWaypointOff()
        local N = tRNG.addBlip(J.position.x, J.position.y, J.position.z, J.blipId, J.blipColour, "Scuba Diving")
        SetNewWaypoint(J.position.x, J.position.y)
        tRNG.createArea("scubajob_" .. J.name, J.position, 250, 15, q, s, t, {})
        h = tRNG.getCustomization()
        local p = math.random(1, 15)
        local O = PlayerPedId()
        ...
        TriggerServerEvent("RNG:scubaSetVehicle", c, R)
        DoScreenFadeIn(2000)
        SetPedScubaGearVariation(PlayerPedId())
            ...
            function()
                while e do
                    if i then
                        ...
                        SetWaypointOff()
                        TriggerServerEvent("RNG:finishScubaJob", c, 0)
                        TriggerServerEvent("RNG:claimScubaReward", c)
                        d = 0
                        c = nil
                        e = false
...
function createRewardObjects(J)
    for z, A in pairs(J.rewardObjects) do
        local T = tRNG.loadModel(A.objectModel)
        ...
        local q = function(r)
            tRNG.drawNativeNotification("Press ~INPUT_PICKUP~ to collect!")
        end
        local s = function(r)
        end
        local t = function(r)
            if IsControlJustReleased(1, 38) and e then
                tRNG.notify("~g~Item collected.")
                d = d + 1
                tRNG.removeMarker(b[r.objectId].objectMarker)
                tRNG.removeArea(b[r.objectId].objectArea)
        ...
        tRNG.createArea("scubajob_" .. J.name .. "_obj_" .. z, A.objectPosition, 3.0, 3.0, q, s, t, {objectId = z})
        b[z] = {}
        b[z].objectId = U
        b[z].objectMarker = V
