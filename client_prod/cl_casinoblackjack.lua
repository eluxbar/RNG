...
RegisterCommand(
    "getcasinotable",
    function()
        local K = GetEntityCoords(tRNG.getPlayerPed())
        ...
        for O = 0, #G.blackjackTables, 1 do
            ...
            tRNG.createDynamicPed(
                ...
                function(Q)
                    ...
                    tRNG.loadAnimDict(dealerAnimDict)
        ...
        while true do
            ...
            if h then
                if IsDisabledControlJustPressed(0, 22) then
                    local W = tRNG.KeyboardInput("Bet Amount", "", 30)
                    if tonumber(W) then
                        W = tonumber(W)
                        if W > 0 then
                ...
                if IsControlJustPressed(0, 201) then
                    if tonumber(z) >= 0 then
                        ...
                        elseif isChairHighRollers(F) and z < B then
                            tRNG.notify("~r~Minimum bet is " .. tostring(getMoneyStringFormatted(B)))
                        elseif z > 100000000000 then
                            tRNG.notify("~r~Maximum bet is £100,000,000,000")
                        else
    ...
    function()
        while true do
            if sittingAtBlackjackTable and A then
                SetPedCapsule(tRNG.getPlayerPed(), 0.2)
                if IsControlJustPressed(0, 202) and not i then
                    ...
                    TaskPlayAnim(tRNG.getPlayerPed(), blackjackAnimDictToLoad, "sit_exit_left", 1.0, 1.0, 2500, 0)
...
RMenu:Get("rngblackjack", "instructions"):SetSubtitle("Blackjack Table")
RMenu.Add(
    "rngblackjack_high",
    "instructions",
    RageUI.CreateMenu("", "test", 1300, 50, "casinoui_cards_blackjack_high", "casinoui_cards_blackjack_high")
)
RMenu:Get("rngblackjack_high", "instructions"):SetSubtitle("High Rollers Table")
RageUI.CreateWhile(
    1.0,
    true,
    function()
        if RageUI.Visible(RMenu:Get("rngblackjack", "instructions")) then
            RageUI.DrawContent(
                ...
                function()
                    RageUI.FakeButton(
                        ...
                        RMenu:Get("rngblackjack", "instructions")
                    )
                end,
                function()
                end
        ...
        if RageUI.Visible(RMenu:Get("rngblackjack_high", "instructions")) then
            RageUI.DrawContent(
                {header = true, glare = false, instructionalButton = true},
                function()
                    RageUI.FakeButton(
                        ...
                        RMenu:Get("rngblackjack_high", "instructions")
                    )
                end,
                function()
                end
...
function showHowToBlackjack(a3)
    ...
    if not isChairHighRollers(a) then
        RageUI.Visible(RMenu:Get("rngblackjack", "instructions"), a3)
    else
        RageUI.Visible(RMenu:Get("rngblackjack_high", "instructions"), a3)
    end
end
...
function blackjackCameraView(a4, Y)
    ...
    local localChairId = getLocalChairIndexFrornglobalChairId(Y)
...
function goToBlackjackSeat(an)
    ...
    c = an
    fVar3 = blackjack_func_217(tRNG.getPlayerPed(), blackjack_func_218(c, 0), 1)
    fVar4 = blackjack_func_217(tRNG.getPlayerPed(), blackjack_func_218(c, 1), 1)
    fVar5 = blackjack_func_217(tRNG.getPlayerPed(), blackjack_func_218(c, 2), 1)
    ...
    TaskGoStraightToCoord(tRNG.getPlayerPed(), ao.x, ao.y, ao.z, 1.0, 5000, ap, 0.01)
...
function startSingleDealing(Y, U, ab, ac, ai, ae)
    ...
    local gender = getDealerGenderFromPed(U)
    if DoesEntityExist(U) then
        local localChairId = getLocalChairIdFrornglobalChairId(Y)
        local nextCard = getCardFromNumber(ac, true)
...
function startDealing(U, ab, ac, Y, ad, ae, az)
    ...
    if DoesEntityExist(U) then
        ...
        if Y <= 1000 then
            ...
            cardOffsetX, cardOffsetY, cardOffsetZ =
                blackjack_func_377(iVar5, getLocalChairIndexFrornglobalChairId(Y), 0)
        else
        ...
        if Y <= 1000 then
            vVar8 = vector3(0.0, 0.0, getTableHeading(blackjack_func_368(Y)))
            cardObjectOffsetRotation = vVar8 + func_376(iVar5, getLocalChairIndexFrornglobalChairId(Y), 0, false)
            SetEntityRotation(
                aw,
...
function startStandOrHit(ab, U, Y, aA)
    chairAnimId = getLocalChairIdFrornglobalChairId(Y)
    ...
    function(Y, tableId)
        if isNearbyBlackjackTableId(tableId) then
            if string.sub(Y, -5) ~= "chips" then
                ...
                localChairId = getLocalChairIdFrornglobalChairId(Y)
                if Y > 99 then
                    TaskPlayAnim(
                        U,
                        "anim_casino_b@amb@casino@games@blackjack@dealer",
...
function betChipsForNextHand(aH, av, aI, ah, aJ, aK)
    ...
    local aM = blackjack_func_374(aH, aI, getLocalChairIndexFrornglobalChairId(ah), aJ)
    ...
    local aP = blackjack_func_373(aH, 0, getLocalChairIndexFrornglobalChairId(ah), aJ)
...
function getNewCardFromMachine(nextCard, Y, ab)
    local aR = tRNG.loadModel(nextCard)
...
function dealerGiveCards(Y, gender, U, cardObj)
    local aT = tostring(getLocalChairIdFrornglobalChairId(Y))
    TaskPlayAnim(
        U,
        "anim_casino_b@amb@casino@games@blackjack@dealer",
...
function blackjack_func_348(aY)
    ...
    if DoesEntityExist(b8) then
        local localChairId = getLocalChairIndexFrornglobalChairId(aY)
...
function getLocalChairIdFrornglobalChairId(ba)
    if ba ~= -1 then
        return ba % 4 + 1
    else
        return 100
...
function getLocalChairIndexFrornglobalChairId(ba)
    if ba ~= -1 then
        return ba % 4
    else
        return 100
