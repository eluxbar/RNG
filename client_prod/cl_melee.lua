...
RegisterCommand(
    "togglemelee",
    function()
        if tRNG.getUserId() == 0 or tRNG.getUserId() == -1 then
            c = not c
            tRNG.notify("Melee Combat: " .. (c and "~g~Enabled" or "Disabled"))
        end
    end
)
function tRNG.enablePunching(d)
    c = d
end
Citizen.CreateThread(
    function()
        tRNG.enablePunching(false)
        while true do
            local e = PlayerPedId()
            local f = tRNG.getPlayerVehicle()
            local g = PlayerId()
    ...
    function(h, i)
        if i then
            ...
            for n, o in pairs(a) do
                tRNG.createArea("boxing_" .. n, o.pos, o.radius, 6, j, l, m, {})
            end
        end
    end
)
    ...
    function(h, i)
        if i then
            ...
            for n, u in pairs(t) do
                tRNG.addMarker(u.x, u.y, u.z, 1.0, 1.0, 1.0, 0, 0, 255, 100, 50, 27, false, false, true)
                tRNG.createArea(n .. "_boxingGloves", u, 1.5, 6, j, l, m, {})
            end
        end
    end
)
...
function putGloves()
    ...
    local x = tRNG.loadModel("prop_boxing_glove_01")
