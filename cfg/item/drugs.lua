...
cocaine_sniff["Take"] = {function(player,choice)
  ...
  if user_id ~= nil then
    if RNG.tryGetInventoryItem(user_id,"Cocaine",1) then
      RNGclient.notify(player,{"~g~Taking Cocaine."})
      TriggerEvent('RNG:RefreshInventory', player)
      TriggerClientEvent('RNG:cocaineEffect', player)
    end
  end
end}
...
heroin_take["Take"] = {function(player,choice)
  local user_id = RNG.getUserId(player)
  if user_id ~= nil then
    if RNG.tryGetInventoryItem(user_id,"Heroin",1) then
      RNGclient.notify(player,{"~g~Taking Heroin."})
      TriggerEvent('RNG:RefreshInventory', player)
      TriggerClientEvent('RNG:heroinEffect', player)
    end
  end
end}
...
lsd_take["Take"] = {function(player,choice)
  local user_id = RNG.getUserId(player)
  if user_id ~= nil then
    if RNG.tryGetInventoryItem(user_id,"LSD",1) then
      RNGclient.notify(player,{"~g~Taking LSD."})
      TriggerEvent('RNG:RefreshInventory', player)
      TriggerClientEvent('RNG:doAcid', player)
    end
  end
end}
...
morphine_choices["Take"] = {function(player,choice)
  local user_id = RNG.getUserId(player)
  if user_id ~= nil then
    if RNG.tryGetInventoryItem(user_id,"Morphine",1) then
      TriggerEvent('RNG:RefreshInventory', player)
      TriggerClientEvent('RNG:applyMorphine', player)
    end
  end
end}
...
taco_choices["Take"] = {function(player,choice)
  local user_id = RNG.getUserId(player)
  if user_id ~= nil then
    if RNG.tryGetInventoryItem(user_id,"Taco",1) then
      TriggerEvent('RNG:RefreshInventory', player)
      TriggerClientEvent('RNG:eatTaco', player)
    end
  end
end}
