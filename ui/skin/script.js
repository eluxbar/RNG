$(document).ready(function(){
  ...
  $('input').change(function(){
    ...
    $.post('https://rng/updateSkin', JSON.stringify({
      value: false,
      // Face
  ...
  $(document).keypress(function(e) {
    ...
    if(e.which == 113){ // A pressed
      $.post('https://rng/rotaterightheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 101){ // E pressed
      $.post('https://rng/rotateleftheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 119 ){ // W pressed
      $.post('https://rng/zoomin', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 115){ // S pressed
      $.post('https://rng/zoomout', JSON.stringify({
        value: 10
      }));
    }
  });
  ...
  $('#skin-tabs skin-label').on('click', function(e){
    ...
    $.post('https://rng/zoom', JSON.stringify({
...
function leftWristComponent() {
    $.post('https://rng/updateLeftWristComponent', JSON.stringify({
...
function leftWristTexture() {
    ...
    $.post('https://rng/updateLeftWristTexture', JSON.stringify({
        componentID: $('.montre .active').attr('data'),
...
function rightWristComponent() {
    $.post('https://rng/updateRightWristComponent', JSON.stringify({
...
function rightWristTexture() {
    ...
    $.post('https://rng/updateRightWristTexture', JSON.stringify({
        componentID: $('.montre-right .active').attr('data'),
