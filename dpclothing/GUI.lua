...
local LastRemoved = {}  -- Table to store the last removed times
local RemovalCooldown = 10 -- Time in seconds to consider as "recently taken off"
local LastMenuCloseTime = 0 -- Time when the menu was last closed
local MenuCooldown = 10 -- Time in seconds to consider as "recently in menu"

-- Function to update the last removed time
local function UpdateLastRemoved(item)
    LastRemoved[item] = GetGameTimer()
end

-- Function to check if an item was removed recently
function WasRecentlyRemoved(item)
    if LastRemoved[item] then
        return (GetGameTimer() - LastRemoved[item]) < (RemovalCooldown * 1000)
    else
        return false
    end
end

-- Function to update the last menu close time
local function UpdateLastMenuCloseTime()
    LastMenuCloseTime = GetGameTimer()
end

-- Function to check if the user was recently in the menu
function WasRecentlyInMenu()
    return (GetGameTimer() - LastMenuCloseTime) < (MenuCooldown * 1000)
end
        ...
        if Button and not Cooldown then	-- If the button is clicked we execute the command, just like if the player typed it in chat.
            if v.Sprite == "gloves" then
                if not LastEquipped["Shirt"] then
                    if not WasRecentlyRemoved("Shirt") then
                        PushedButton(k)
                        ExecuteCommand(v.Command)
                    else
                        Notify(Lang("RecentlyTakenOff"))
                    end
                else
                    Notify(Lang("NoShirtOn"))
                end
            else
                if not WasRecentlyRemoved(v.Sprite) then
                    PushedButton(k)
                    ExecuteCommand(v.Command)
                else
                    Notify(Lang("RecentlyTakenOff"))
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if not Config.GUI.Toggle then
            if IsControlPressed(1, Config.GUI.Key) and not tRNG.isInComa() then
                local Ped = PlayerPedId() 
                if Check(Ped) then MenuOpened = true end
            else MenuOpened = false end
            if IsControlJustPressed(1, Config.GUI.Key) and not tRNG.isInComa() then
                local Ped = PlayerPedId() 
            ...
            elseif IsControlJustReleased(1, Config.GUI.Key) then
                if Check(Ped) then 
                    MenuOpened = false 
                    SoundPlay("Close") 
                    UpdateLastMenuCloseTime() -- Update the last menu close time
                end
            end
        else
            if IsControlJustPressed(1, Config.GUI.Key) and not tRNG.isInComa() then
